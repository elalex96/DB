...
-- Description:	/*SE AGREGO LA VALIDACION PARA DESCARTAR PROCESOS DE MURPHY*/
-- =============================================
CREATE PROCEDURE [dbo].[SP_MPY_MM_ValidacionEliminacionProceso] 
...
BEGIN
	...
	BEGIN 
		/*VALIDACION MPY*/
		DECLARE @MPYAF INT = (SELECT COUNT(IdAceptacionFactura) FROM dbo.MPY_MM_AceptacionFactura WHERE IdAceptacionPedido = @IdProceso)

		IF ISNULL(@MPYAF,0) = 0
		BEGIN
		/*VALIDAR EXISTE UNA ACEPTACIÓN DE FACTURA OBTENER EL IDELIMINACION*/
		
		SELECT @IDELIMINADO= IdEliminado FROM dbo.MPY_MM_AceptacionFactura WHERE IdAceptacionPedido = @IdProceso
		...
		IF ISNULL(@IDELIMINADO,0) =0  		
			SELECT TOP 1 @IDELIMINADO= IdEliminado FROM dbo.MPY_MM_AceptacionCartaPCN WHERE IdAceptacionPedido = @IdProceso ORDER BY CreadoEl DESC 
	...
	BEGIN 
		...
		DECLARE @MPYAP INT = (SELECT COUNT(IdAceptacionCartaPCN) FROM dbo.MPY_MM_AceptacionCartaPCN WHERE IdAceptacionPedido = @IdProceso)
