...
-- =============================================
-- Author:		DANIEL AC
-- Create date: 14/04/26
-- Description:	SE SEPARAN LOS DESTINATARIOS DE NOTIFICACIÓN POR PERFIL (PROCURA: ADMINISTRADOR Y COMPRAS / PETROVENDOR: ADMINISTRADOR Y VENTAS) Y LAS URLS SE OBTIENEN DINÁMICAMENTE DESDE TA_Dominios
-- =============================================
...
BEGIN
	...
	@UrlPetrovendor NVARCHAR(500)= (SELECT TOP 1 Url FROM TA_Dominios  (NOLOCK) WHERE IdDominio= 1),
	@UrlProcura NVARCHAR(500)= (SELECT TOP 1 Url FROM TA_Dominios  (NOLOCK) WHERE IdDominio= 2)
	...
	--LOS DATOS DE LOS USUARIOS A LOS QUE SE ENVIARAN (VENTAS,COMPRAS Y ADMIN) DEL PROVEEDOR
	...
	BEGIN
		...
		SELECT
			...
			CONCAT(@UrlPetrovendor,'01Proveedores/CO_CotizacionDetalle.aspx?oferta=' , CAST(PO.IdPeticionOferta AS NVARCHAR(100))),
			US.IdUsuarioADINCO,
		...
		WHERE PO.IdSolicitudPedido = @IdSolicitudPedido
			AND (US.IdTipoUsuario = 3 OR US.IdTipoUsuario = 4) --> CTE 3 ADMIN Y 4 VENTAS
		...
		SELECT
			...
			PO.IdPeticionOferta,
			PO.IdSolicitudPedido,
			CONCAT(@UrlProcura,'02Proveedores/DetalleOferta.aspx?solped=' , CAST(@IdSolicitudPedido AS NVARCHAR(100))),
			US.IdUsuarioADINCO,
			'PROVEEDOR-PROCURA'
		FROM dbo.MM_PeticionOferta AS PO  (NOLOCK)
			...
			and  TANN.IdCorreo = @IdCorreo
		WHERE PO.IdSolicitudPedido = @IdSolicitudPedido
			AND (US.IdTipoUsuario = 3 OR US.IdTipoUsuario = 5)  --> CTE 3 ADMIN Y 5 COMPRAS
			and	US.Activo = 1
