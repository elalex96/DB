...
(
    SELECT 1
    FROM dbo.sysobjects
    WHERE name = 'SP_MM_ConsultaValidacionEliminacion_SolicitudPedido'
)
    DROP PROCEDURE SP_MM_ConsultaValidacionEliminacion_SolicitudPedido;   
	
GO
/****** Object:  StoredProcedure [dbo].[SP_MM_ConsultaValidacionEliminacion_SolicitudPedido]    Script Date: 08/11/2024 06:33:35 a. m. ******/
SET ANSI_NULLS ON
GO
...
-- Description: CONSULTAR PROCESO DE PROCURA PARA POSIBLE ELIMINACIÓN  DE UNA SOLICITUD PEDIDO -MODIFICACIÓN
-- =============================================
-- =============================================
-- Author:	DANIEL AC
...
CREATE  PROCEDURE [dbo].[SP_MM_ConsultaValidacionEliminacion_SolicitudPedido] 
...
BEGIN
	...
	WHERE O.IdTipoOperacion = 9 --> APROBACIÓN DE PEDIDO
	 ...
	 AND O.IdEstatusOperacion=2 ---> QUE ESTEN APROBADOS 
	...
	INNER JOIN MM_Pedidos AS PG (NOLOCK)
		ON P.IdPedido = PG.IdIdentificador 
		AND PG.IdProveedorCliente = @IDPROVEEDOR
	INNER JOIN #PROCESO AS PSP (NOLOCK)
		ON P.IdPedido = PSP.ID_PROCESO
	LEFT JOIN  MM_TipoPedido AS TP (NOLOCK)
		ON PG.IdTipoPedido = TP.IdTipoPedido
	...
	WHERE 	
	 ...
	 /*RECEPCIÓN DE FACTURA DE PETROVENDOR CON CARTA EXCLUIDA*/
	...
	INNER JOIN MM_Pedido AS P (NOLOCK)
		ON  AP.IdPedido = P.IdPedido  
	INNER JOIN RelacionCartaCNPedido RCCN (NOLOCK)
		ON AP.IdAceptacionPedido = RCCN.IdAceptacionPedido
		AND AP.IdPedido = RCCN.IdPedido
		AND RCCN.PedirCarta = 0
	INNER JOIN S_Proveedor AS PV (NOLOCK)
		ON P.IdSubcontratista	 = PV.IdProveedor
	INNER JOIN MM_Pedidos AS PG (NOLOCK)
		ON P.IdPedido = PG.IdIdentificador
	...
	FROM MM_AceptacionPedido AS AP(NOLOCK)
	INNER JOIN #PROCESO AS PAC (NOLOCK)
		ON AP.IdAceptacionPedido = PAC.ID_PROCESO
	INNER JOIN MM_Pedido AS P (NOLOCK)
		ON  AP.IdPedido =  P.IdPedido 	
	INNER JOIN S_Proveedor AS PV (NOLOCK)
		ON  P.IdSubcontratista	= PV.IdProveedor
	...
	INNER JOIN #PEDIMENTO_GASTOS PG (NOLOCK)
