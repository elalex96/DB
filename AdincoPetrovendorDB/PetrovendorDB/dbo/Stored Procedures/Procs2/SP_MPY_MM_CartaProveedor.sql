...
BEGIN  
 ...
 SET @IdTipoRegimen =(SELECT IdTipoRegimen FROM S_PROVEEDOR (NOLOCK) WHERE IdProveedor = @IdProveedor);
...
SET @PROVEEDOR_DEA = 0; 
 ...
 SET @CANT_TMAT = (SELECT 
							...
							WHERE AP.IdAceptacionPedido = @IdPedido 
								AND VP.IdTipoMaterialServicio = 1);
 ...
 SET @CANT_TSER = (SELECT 
							...
							LEFT JOIN dbo.MPY_MM_AceptacionPedido AS AP (NOLOCK)
								ON APD.IdAceptacionPedido  = AP.IdAceptacionPedido
							WHERE AP.IdAceptacionPedido = @IdPedido 
								AND VP.IdTipoMaterialServicio = 2);  
