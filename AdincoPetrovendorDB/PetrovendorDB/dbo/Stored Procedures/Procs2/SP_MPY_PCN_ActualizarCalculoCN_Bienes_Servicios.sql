...
-- =============================================
CREATE procedure [dbo].[SP_MPY_PCN_ActualizarCalculoCN_Bienes_Servicios]
				...
				BEGIN
				 /*PCN = 1 POR DEFAULT ES 1 */
				  ...
				  IF @IdCriterio <> 2 /*CRITERIO 1 Y 2 NO LLEVAN FRACCION ARRANCELARIA*/
					...
					UPDATE MPY_MM_PCN_ValoresPesos
					...
					SET @PCN_PedidoDetalleAll = (SELECT COUNT(APD.IdAceptacionPedidoDetalle) 
												 FROM MPY_MM_AceptacionPedidoDetalle APD
												 INNER JOIN MPY_MM_AceptacionPedido AP ON AP.IdAceptacionPedido = APD.IdAceptacionPedido 
				...
				BEGIN		
					...
					*/
					...
					SET @PCNMjxVMj =(SELECT SUM(ISNULL(MU.PCNM_Utilizado,0) * ISNULL(MU.VM_ValorFactura,0)) AS PCNMixVMi
								FROM MPY_MM_PCN_MaterialesUtilizados MU				
					...
					SET @VNMOi = (SELECT ISNULL(VNMO_SueldoNacional,0) FROM MPY_MM_PCN_ValoresPesos WHERE IdValoresEnPesosPedidoDetalle = @IdValoresEnPesosPedidoDetalle)
					SET @VMOi = (SELECT ISNULL(VMO_Sueldo,0) FROM MPY_MM_PCN_ValoresPesos WHERE IdValoresEnPesosPedidoDetalle = @IdValoresEnPesosPedidoDetalle)
