...
BEGIN
	...
	SELECT u.IdUsuario,
		...
		WHERE UP.IdProveedor = @IdProveedor AND U.IdTipoUsuario = 3 
		AND U.Activo = 1 AND U.IsEliminado = 0) > 0 THEN U.Activo ELSE 0 END AS Activo,
		--u.Activo,
		u.IsEliminado,
	...
	ON p.IdProveedor = up.IdProveedor	
		LEFT JOIN dbo.S_ImagenPerfil i ON i.IdProveedor = up.IdProveedor
	WHERE u.Correo = @Correo AND u.IsEliminado = 0
END