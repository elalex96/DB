...
BEGIN
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor = prov.IdProveedor 
        JOIN JA_ComentarioBase comBase	(NOLOCK)
            ON mensajes.IdPrimario = comBase.IdComentarioBase 
    WHERE (
	...
	--[JA_ComentariosBasesConvocatoria TipoMensaje = 1]
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor = prov.IdProveedor 
        JOIN JA_ComentarioRespuesta resp	(NOLOCK)
            ON  mensajes.IdPrimario = resp.IdComentarioRespuesta 
    WHERE (
              mensajes.Enviado = 0 -- no se le ha enviado el mensaje al usuario
              OR (
                     mensajes.Enviado = 1
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor = prov.IdProveedor 
			AND mensajes.IdProveedor = @IdProveedor
        JOIN JA_ComentariosBasesConvocatoria com	(NOLOCK)
            ON mensajes.IdPrimario = com.IdComentarioBases 
    WHERE (
              mensajes.Enviado = 0 -- no se le ha enviado el mensaje al usuario
              OR (
                     mensajes.Enviado = 1
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor = prov.IdProveedor 
			AND mensajes.IdProveedor = @IdProveedor
        JOIN JA_ComentariosBasesConvocatoriaRespuesta resp	(NOLOCK)
            ON mensajes.IdPrimario = resp.IdRespuesta 
    WHERE (
              mensajes.Enviado = 0 -- no se le ha enviado el mensaje al usuario
              OR (
                     mensajes.Enviado = 1
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor = prov.IdProveedor 
			AND mensajes.IdProveedor = @IdProveedor
        JOIN JA_EncabezadoComentarios enc	(NOLOCK)
            ON mensajes.IdPrimario = enc.IdEncabezado 
    WHERE (
              mensajes.Enviado = 0 -- no se le ha enviado el mensaje al usuario
              OR (
                     mensajes.Enviado = 1
    ...
    FROM dbo.Ja_MensajesPendientesComentarios mensajes	(NOLOCK)
        JOIN dbo.S_Proveedor prov	(NOLOCK)
            ON mensajes.IdProveedor	 = prov.IdProveedor 
			AND mensajes.IdProveedor = @IdProveedor
        JOIN JA_TopicAclaraciones junta	(NOLOCK)
            ON mensajes.IdPrimario = junta.IdTopic 
    WHERE (
              mensajes.Enviado = 0 -- no se le ha enviado el mensaje al usuario
              OR (
                     mensajes.Enviado = 1
