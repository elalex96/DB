...
BEGIN
	...
	SELECT TOP 1 
		@LineaPresupuesto = 
		CONCAT(
		   ISNULL('Instalaci�n: ' + INS.NombreInstalacion  COLLATE Modern_Spanish_CI_AS,''), 
	...
	LEFT JOIN Adinco.dbo.CO_Instalacion ins ON ins.IdInstalacion = lpm.IdInstalacion
	WHERE P.IdPedido = @IdPedido
	...
	INNER JOIN dbo.MM_Pedidos AS PG ON P.IdPedido = PG.IdIdentificador AND PG.IdProveedorCliente = P.IdProveedorCompras	AND PG.IdTipoPedido IN (2, 4, 6) ---(Mer, AD, OT)
