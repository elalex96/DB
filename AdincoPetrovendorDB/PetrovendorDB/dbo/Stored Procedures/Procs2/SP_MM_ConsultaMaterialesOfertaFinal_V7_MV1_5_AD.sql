...
-- Description:	Agrege condicion de no incluir cantidades de pedido eliminados 
-- =============================================
-- Author:		Jose Roman
...
CREATE  PROCEDURE [dbo].[SP_MM_ConsultaMaterialesOfertaFinal_V7_MV1_5_AD]  
	...
	/*----------------------------------------------------------*/  
	--- [SP_MM_ConsultaMaterialesOfertaFinal_V6] 12342 12417
AS
BEGIN
	...
	SET  @MATERIALES_EN_POD_ACTUAL  = (SELECT COUNT(IdPeticionOferta)
									   FROM MM_PeticionOferta AS PO LEFT JOIN dbo.MM_SolicitudPedido sp ON PO.IdSolicitudPedido = sp.IdSolicitudPedido
	...
	SET @ADJUDICACION_PARCIAL  = (SELECT AdjudicableParcialmente FROM MM_SolicitudPedido WHERE IdSolicitudPedido= @IdSolicitudPedido AND IdContrato = @IdContrato)
	 ...
	 ELSE 
		...
		SET @MOSTRAR_PRECIOS = 'SI' --(SELECT (( case  when  (DATEDIFF(MINUTE,@OFERTA_FINALIZACION_LICITACION, GETDATE()))  <= 0 then  'NO' ELSE 'SI' END )))

	
		SET @PeticionFinalizada = (SELECT COUNT(IdPedido) FROM MM_Pedido WHERE IdSolicitudPedido = @IdSolicitudPedido AND ISNULL(IdEstatusEliminado,0)<>1) --> Descartar pedidos eliminados 
							...
							BEGIN 
							      ...
							      DECLARE @ETIQUETA_MENOR NVARCHAR(200)
									...
									IF @PRECIO_UNITARIO_ACTUAL = @Cantidad_PU_MejorCosto  OR @MejorOferta = 1
										...
										BEGIN
											SET @TABLA = @TABLA + '<td class="item_peticion" bgcolor="'+@COLOR_PU_COSTO_MENOR+'" style="text-align:center" data-idPOD="'+CAST(@ID_POD AS nvarchar(50))+'" data-Proveedor="'+CAST(@IdProveedorActual AS nvarchar(50))+'"  title="Precio Unitario del Material/Servicio. Seleccione para ver más detalles">' 
									...
									ELSE
										...
										BEGIN
											SET @TABLA = @TABLA + '<td class="item_peticion" data-idPOD="'+CAST(@ID_POD AS nvarchar(50))+'"  style="text-align:center"  data-Proveedor="'+CAST(@IdProveedorActual AS nvarchar(50))+'" title="Precio Unitario del Material/Servicio. Seleccione para ver más detalles">'	
											SET @ETIQUETA_MENOR = ''
										END
										...
										BEGIN 
											...
											IF @ADDLISTAPEDIDO = 1
												BEGIN
													...
													SET @TABLA = @TABLA +'<h4><span class="semi-bold">$'+ CAST(CONVERT(NVARCHAR(MAX), CAST(@PRECIO_UNITARIO_ACTUAL AS money), 1) AS NVARCHAR(MAX))+'</span></H4><span class="text-black small-text">'+@TIPO_MONEDA_TEXT+' ' +@ICON_ADDPEDIDOTEMP+' '+'<span class="badge badge-success" title="Cantidad agregada a la orden de compra">'+CAST(@ADD_CANTIDADTEMP AS nvarchar(50))+'</span></span>'  
									...
									ELSE
										BEGIN 
											...
											IF @ADDLISTAPEDIDO = 1
												BEGIN
													SET @TABLA = @TABLA +'<h4><span class="semi-bold">$'+ CAST(CONVERT(NVARCHAR(MAX), CAST(@PRECIO_UNITARIO_ACTUAL AS money), 1) AS NVARCHAR(MAX))+'</span></h4>'+'<span class=" text-black small-text">'+@TIPO_MONEDA_TEXT+' ' +@ICON_ADDPEDIDOTEMP+' '+'<span class="badge badge-success" title="Cantidad agregada">'+CAST(@ADD_CANTIDADTEMP AS nvarchar(50))+'</span>&nbsp;'+@ICON_VENCIDO +'</span>'  
						 						END 
