USE [Petrovendor]
GO
IF OBJECT_ID('Petrovendor..SP_PC_ConsultaAprobacionesPedimentosComprobantes_CD') IS NOT NULL
BEGIN
DROP PROCEDURE SP_PC_ConsultaAprobacionesPedimentosComprobantes_CD;
END
GO
-- =============================================  
-- Author:  <Alexander Gomez>  
-- Create date: <02/09/20202>  
-- Description: <Consulta de las aprobaciones de pedimentos comprobantes de compra directa, se agrego la columna estatus al filtro de b�squeda>  
-- ============================================= 
-- =============================================  
-- Author:  <Daniel AC>  
...
-- Description: <Se filtra por usuario creador o usuario aprobador de la operaci�n>  
-- =============================================   
CREATE PROCEDURE [dbo].[SP_PC_ConsultaAprobacionesPedimentosComprobantes_CD]  
 ...
 SET @AllRecords = (  
	  ...
	  FROM dbo.FI_AceptacionPedido_PedimentoComprobante AS APC  
	   JOIN dbo.TA_Operacion AS OP  
		...
		AND OP.IdTipoOperacion = 19  --> APROBACI�N DE COMPRA DIRECTA/PEDIMENTO DIRECTA
	  ...
	  AND APC.Activo = 1
	  AND (dbo.FN_EsAprobadorPedimentoComprobanteCD(OP.IdOperacion,@IdUsuario) = 1 OR APC.CreadoPor = @IdUsuario)
	  AND 
		   (CAST(APC.IdPedimentoComprobante AS NVARCHAR) LIKE	'%' + @Buscar + '%'			  OR  
