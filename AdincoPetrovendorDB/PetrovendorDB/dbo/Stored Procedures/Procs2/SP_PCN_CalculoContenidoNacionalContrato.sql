...
-- =============================================
CREATE PROCEDURE SP_PCN_CalculoContenidoNacionalContrato
...
BEGIN
	...
	/*AGRUPADO POR ACTIVIDAD PCN DE CADA ACTIVIDAD TOTAL*/
	SET @CONTRATO = (SELECT NumeroContrato FROM Adinco.dbo.CO_Contrato WHERE IdContrato = @IDCONTRATO)

	CREATE TABLE #ACTIVIDAD_TOTAL(CodigoCatalogo NVARCHAR(MAX), NombreActividad NVARCHAR(MAX), CN FLOAT, MontoAcumulado MONEY)
	
	INSERT INTO #ACTIVIDAD_TOTAL
	SELECT CodigoCatalogo, 
	NombreActividad AS MaterialCotizadoTextoC,
	CASE WHEN  ISNULL(SUM(CN),0) > 0 THEN 
	ROUND((SUM(CN)/SUM(MontoAcumulado)),3)
	...
	SET @TOTALCN = (SELECT SUM(CN) FROM #ACTIVIDAD_TOTAL)

	SET @TOTALFACTURADO = (SELECT SUM(MontoAcumulado) FROM #ACTIVIDAD_TOTAL)

	SELECT @CONTRATO AS CONTRATO,@TOTALCN AS TOTALCN, @TOTALFACTURADO AS TOTALFACTURADO
END
