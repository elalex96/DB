...
FROM Adinco.dbo.CO_PeriodoContrato  AS COPC (NOLOCK)
      JOIN Adinco.dbo.CO_ProgramaActividad AS COPA  (NOLOCK)
		ON COPC.IdPeriodo = COPA.IdPeriodoContrato
      JOIN Adinco.dbo.CO_Presupuesto AS COP (NOLOCK)
		ON COPA.IdProgramaActividad = COP.IdProgramaActividad 
		AND 1 = COP.Actual
	  JOIN Adinco.dbo.CO_LineaPresupuestoMes AS LPM (NOLOCK)
		ON COP.IdPresupuesto  = LPM.IdPresupuesto
...
INSERT INTO [dbo].[MM_SolicitudPedido]
	...
	WHERE 
	PDI.PURCHASING_DOCUMENT = @Purchasing 
	AND IDCONTRATO = @IdContrato 
