...
-- =============================================
CREATE  PROCEDURE [dbo].[SP_TA_ConsultarDatosReporteOrdenCompra]
...
BEGIN
	...
	DECLARE @PROVEEDOR_COMPRAS INT = (SELECT IdProveedorCompras FROM MM_Pedido P WHERE IdSubcontratista = @IdProveedor AND IdPedido = @IdPedido)	


	DECLARE @PROVEDORACTUAL VARCHAR(MAX) = (SELECT CONCAT(RazonSocial, RegimenCapital) FROM S_Proveedor WHERE IdProveedor = @PROVEEDOR_COMPRAS)
	...
	DECLARE @PROVEDORACTUALDOMICILIO VARCHAR(MAX) = (SELECT CONCAT('Col.',DF.Colonia,' ','Calle.',DF.NombreViabilidad,' ','N°Interior.', DF.NoExterior,' ','N°Exterior.', DF.NoInterior,' ','CP.',DF.CodigoPostal)
													...
													--AND DF.Activo = 1
													WHERE PV.IdProveedor = @PROVEEDOR_COMPRAS)
    ...
    DECLARE @EXISTEN_CONDICIONES_PAGO INT = (SELECT COUNT(CP.IdCondicionPago) FROM PV_CondicionesPago CP
										    ...
										    WHERE CSC.IdContratista = @IdProveedor AND CSC.IdSubContratista = @PROVEEDOR_COMPRAS)
	...
	*/
	...
	PV.Telefono,
	U.Correo,
	PV.RazonSocial +' ' + PV.RegimenCapital AS Cliente,
	...
	O.IdProveedor = @PROVEEDOR_COMPRAS   AND 
	P.IdPedido = @IdPedido 
	GROUP BY 
	O.IdTipoOperacion,
	PG.IdPedido, 
