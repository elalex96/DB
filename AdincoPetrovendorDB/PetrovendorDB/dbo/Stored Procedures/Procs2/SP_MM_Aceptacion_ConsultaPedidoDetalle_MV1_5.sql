...
(
    SELECT 1
    FROM dbo.sysobjects
    WHERE name = 'SP_MM_Aceptacion_ConsultaPedidoDetalle_MV1_5'
)
    DROP PROCEDURE SP_MM_Aceptacion_ConsultaPedidoDetalle_MV1_5;
GO
SET ANSI_NULLS ON
GO
...
-- Description: Se muestra el idmaterial vendido 
-- =============================================  
...
AS  
     BEGIN
          ...
          CONCAT( 'No. Material: ',PD.IdMaterialVendedor, ' ',
          M.DescripcionCorta,
          ...
          FROM dbo.MM_Pedido AS P WITH (NOLOCK)
              ...
              LEFT JOIN dbo.MM_AceptacionPedido AP WITH (NOLOCK) 
				ON AP.IdPedido=P.IdPedido AND ISNULL(AP.IdEstatusEliminado,0)<>1 --> QUE LA ACEPTACIÓN NO ESTE ELIMINADA  
              LEFT JOIN dbo.MM_AceptacionPedidoDetalle APD WITH (NOLOCK) 
