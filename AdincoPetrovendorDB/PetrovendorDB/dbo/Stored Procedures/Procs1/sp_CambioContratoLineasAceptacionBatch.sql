...
CREATE PROCEDURE sp_CambioContratoLineasAceptacionBatch
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            INSERT INTO dbo.HistoricoCambioContrato (IdSolicitudPedido, IdContratoOld, FechaModificado)
    ...
    ELSE
    BEGIN
        INSERT INTO dbo.HistoricoCambioLineaRegistroPetrov (IdSolicitudPedido, IdRegistro, IdLineaPresupuestoOld, FechaModificado)
        SELECT @IdSolicitudPedido,
               r.IdRegistro,
               r.IdLineaPresupuestoMes,
               GETDATE()
        FROM dbo.CO_Registro r
        WHERE r.IdAceptacionPedidoDetalle = @IdAceptacionPedidoDetalle

        UPDATE r
        SET r.IdLineaPresupuestoMes = @LineaCambiar
        FROM dbo.CO_Registro r
        WHERE IdAceptacionPedidoDetalle = @IdAceptacionPedidoDetalle

		UPDATE apdi
		SET apdi.IdLineaPresupuesto = @LineaCambiar
		FROM dbo.MM_AceptacionPedidoDetalleInstalacion apdi
		WHERE apdi.IdAceptacionPedidoDetalle = @IdAceptacionPedidoDetalle

        INSERT INTO dbo.HistoricoCambioLineaRegistroAdinco (IdSolicitudPedido, IdRegistro, IdProgramaOld, FechaModificado)
        SELECT @IdSolicitudPedido,
               r.IdRegistro,
               r.IdPrograma,
               GETDATE()
        FROM Adinco.dbo.CO_Registro r
            INNER JOIN dbo.CO_RelacionRegistroAdinco rel
                ON rel.IdRegistroAdinco = r.IdRegistro
            INNER JOIN dbo.CO_Registro rr
                ON rr.IdRegistro = rel.IdRegistroPetrovendor
        WHERE rr.IdAceptacionPedidoDetalle = @IdAceptacionPedidoDetalle

        UPDATE r
        SET r.IdPrograma = @LineaCambiar
        FROM Adinco.dbo.CO_Registro r
            INNER JOIN dbo.CO_RelacionRegistroAdinco rel
                ON rel.IdRegistroAdinco = r.IdRegistro
            INNER JOIN dbo.CO_Registro rr
