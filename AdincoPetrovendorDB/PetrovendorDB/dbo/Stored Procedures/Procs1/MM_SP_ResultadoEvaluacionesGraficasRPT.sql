...
-- Description:	ACTUALICE SP CON LA VERSIÓN DE JOSE MM_SP_ResultadoEvaluaciones
-- =============================================
CREATE PROCEDURE [dbo].[MM_SP_ResultadoEvaluacionesGraficasRPT]
...
BEGIN
	...
	SET NOCOUNT ON;
	CREATE TABLE #ResultadosProveedores(IdRow INT, NombreProveedor NVARCHAR(MAX),IdProveedor INT, RCF NVARCHAR(MAX), IdPeticionOferta INT, MejorOferta BIT, PrecioTotal FLOAT, ResultadoEG FLOAT, ResultadoET FLOAT, ResultadoEvaluaciones FLOAT, ResultadoEconomico FLOAT, Total NVARCHAR(MAX), Cotizado NVARCHAR(MAX),ValorEvaTE FLOAT, ValorEvaEC FLOAT)
	...
	DECLARE @VALOREVATE INT= (SELECT PorcentajeET FROM dbo.TA_DocMatrizOperacion WHERE IdOperacion=@IdSolicitudPedido)
	DECLARE @VALOREVAEC INT= (SELECT PorcentajeEC FROM dbo.TA_DocMatrizOperacion WHERE IdOperacion=@IdSolicitudPedido)
	INSERT INTO #ResultadosProveedores
		 SELECT ROW_NUMBER() OVER(ORDER BY P.IdProveedor ASC) AS Row#, P.RazonSocial +' '+P.RegimenCapital AS Razonsocial, P.IdProveedor,RFC, PO.IdPeticionOferta,0,0,0,0,N'',N'',N'',N'',@VALOREVATE,@VALOREVAEC
	...
	begin
		UPDATE rp
			SET rp.ResultadoEvaluaciones = ((rp.ResultadoEG * @PorcEvaluacionG / 100) + (rp.ResultadoET * @PorcEvaluacionT / 100))
			FROM #ResultadosProveedores rp

		UPDATE rp
			SET rp.ResultadoEvaluaciones = (CAST(rp.ResultadoEvaluaciones AS FLOAT) * @PorcentajeET) / 100
			FROM #ResultadosProveedores rp
	END
    ELSE
    BEGIN
		UPDATE rp
			SET rp.ResultadoEvaluaciones = (CAST(rp.ResultadoEG AS FLOAT) * @PorcentajeET) / 100
			FROM #ResultadosProveedores rp
	END
    

	UPDATE rp
		SET rp.Total = (CAST(rp.ResultadoEvaluaciones AS FLOAT) + CAST(rp.ResultadoEconomico AS FLOAT))
		FROM #ResultadosProveedores rp
		WHERE rp.PrecioTotal <> 0
	...
	UPDATE rp
		SET rp.ResultadoEconomico = ROUND(CAST(rp.ResultadoEconomico AS FLOAT),2,1),
			rp.Total = ROUND(CAST(rp.Total AS FLOAT),2,1)
		FROM #ResultadosProveedores rp
		WHERE rp.PrecioTotal <> 0

	UPDATE rp
		SET rp.ResultadoEvaluaciones = ROUND(CAST(rp.ResultadoEvaluaciones AS FLOAT),2,1)
		FROM #ResultadosProveedores rp
