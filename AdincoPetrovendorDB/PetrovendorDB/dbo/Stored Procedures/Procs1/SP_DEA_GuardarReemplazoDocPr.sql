...
-- =============================================
CREATE PROCEDURE [dbo].[SP_DEA_GuardarReemplazoDocPr]
...
BEGIN
	...
	INNER JOIN dbo.DEA_Documento_S3 D ON D.IdDocumentoTabla=PR.IdAjuntoPr AND d.IdTipoDocumento=1 --> AdjuntoPR (SELECT * FROM dbo.DEA_TipoDocumento)
    ...
    SET IdTipoDocumento =1, --> AdjuntoPR (SELECT * FROM dbo.DEA_TipoDocumento)
	...
	SELECT 'SUCCESS' AS Response ,D.IdDocumento AS IdDocumentoPr
    FROM dbo.DEA_AdjuntoPR PR
	INNER JOIN dbo.DEA_Documento_S3 D ON D.IdDocumentoTabla=PR.IdAjuntoPr AND d.IdTipoDocumento=1 --> AdjuntoPR (SELECT * FROM dbo.DEA_TipoDocumento)
    WHERE PR.IdSolicitudPedido = @IdSolicitudPedido;
