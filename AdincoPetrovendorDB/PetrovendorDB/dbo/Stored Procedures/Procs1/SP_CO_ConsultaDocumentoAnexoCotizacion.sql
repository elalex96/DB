...
AS
	BEGIN
		...
		IF @TipoDocumento = 'Fianza'
			BEGIN
				SELECT	F.IdDocFianza, F.NombreDoc, '' AS Documento, F.Carpeta, F.Identificador, F.Extension, F.Mime
		...
		IF @TipoDocumento = 'Bases'
			BEGIN
				SELECT	B.IdDocBases, B.NombreDoc, '' AS Documento, B.Carpeta, B.Identificador, B.Extension, B.Mime
		...
		IF @TipoDocumento = 'Anexo'
			BEGIN
				SELECT	SP.IdDocumento, SP.NombreDoc, '' AS Documento, SP.Carpeta, SP.Identificador, SP.Extension ,
						SP.Mime
