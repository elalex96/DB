...
CREATE PROCEDURE sp_CambioContratoLineasBatch @IdSolicitudPedidoDetalle INT, 
                                              @IdContrato               INT, 
                                              @IdPeriodo                INT, 
                                              @IdPresupuesto            INT, 
                                              @LineaCambiar             INT
AS
    BEGIN
        DECLARE @IdProveedor INT, @IdContratoOld INT, @IdSolicitudPedido INT;
            ...
            BEGIN
                ...
                (
                    ...
                    FROM Adinco.dbo.AP_PerfilUsuario AS PU
                         INNER JOIN Adinco.dbo.AP_Perfil ON PU.PerfilID = AP_Perfil.IdPerfil
                         INNER JOIN Adinco.dbo.AP_Rol ON AP_Perfil.IdRol = AP_Rol.IdRol
                         INNER JOIN Adinco.dbo.CO_Contrato ON AP_Perfil.IdContrato = CO_Contrato.IdContrato
                         INNER JOIN Adinco.dbo.CO_AreaContractual ON CO_Contrato.IdAreaContractual = CO_AreaContractual.IdAreaContractual
                         INNER JOIN Petrovendor.dbo.S_UsuarioProveedor uProv ON CO_Contrato.IdContrato = uProv.IdContrato
                         INNER JOIN Petrovendor.dbo.S_Proveedor prov ON uProv.IdProveedor = prov.IdProveedor
                         INNER JOIN Adinco.dbo.CO_PeriodoContrato per ON per.IdContrato = CO_Contrato.IdContrato
                    ...
                    BEGIN
                        ...
                        UPDATE axml
                          SET 
                              axml.IdContrato = @IdContrato
                        FROM dbo.FI_Factura f
                             INNER JOIN dbo.MM_AceptacionFactura af ON af.IdFactura = f.IdFactura
                             INNER JOIN dbo.MM_AceptacionPedido ap ON ap.IdAceptacionPedido = af.IdAceptacionPedido
                             INNER JOIN dbo.MM_Pedido p ON p.IdPedido = ap.IdPedido
                             INNER JOIN dbo.FI_ArchivoXml axml ON axml.IdFactura = f.IdFactura
                        WHERE p.IdSolicitudPedido = @IdSolicitudPedido;
                        UPDATE pc
                          SET 
                        ...
                        FROM dbo.FI_Factura f
                             ...
                             INNER JOIN dbo.FI_PedimentoComprobante pc ON pc.IdPedidoGeneral = ps.IdPedido
                        WHERE p.IdSolicitudPedido = @IdSolicitudPedido;
