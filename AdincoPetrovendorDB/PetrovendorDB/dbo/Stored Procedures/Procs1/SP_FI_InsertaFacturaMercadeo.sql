...
CREATE PROCEDURE [dbo].[SP_FI_InsertaFacturaMercadeo] 
	@Serie	nvarchar(MAX) ,
	@Folio	nvarchar(MAX) ,
	@Fecha	datetime ,
	@Sello	nvarchar(MAX) ,
...
IF @IdFactura IS NULL OR @IdFactura = 0
	...
	BEGIN 
			  ...
			  INSERT INTO [dbo].[FI_Factura]
				...
				SELECT CAST( @IdFactura  as nvarchar)  AS INSERTADO ,
					 'La factura ' + @Serie +'-'+ @Folio + ' se ha registrado correctamente con el id ' + CAST( @IdFactura  as nvarchar)  as MSG,
					 @IdFactura
	...
	BEGIN 
			...
			SELECT CAST( @IdFactura  as nvarchar)  AS INSERTADO , 
				'La factura ' + @Serie +'-'+ @Folio + ' se ha Actualizado correctamente con el id ' + CAST( @@IDENTITY  as nvarchar)  as MSG,
				@IdFactura
