...
AS
    BEGIN
        ...
        DECLARE @PrecioUnitario_Actual FLOAT, @Disponibilidad_Actual FLOAT, @IdMoneda_Actual INT ,
            ...
            =   '', @Subtotal FLOAT, @DisponibilidadNEW DECIMAL (20, 3), @MaterialCotizadoTextC NVARCHAR (MAX) ,
            ...
            BEGIN
                ...
                UPDATE  [dbo].[MM_PeticionOfertaDetalle]
                   SET  [PrecioUnitario] = CAST(@PrecioUnitario AS NUMERIC (18, 2)) ,
                        ...
                        [ModificadoEl] = GETDATE (), [IdMoneda] = @IdMoneda, [Disponibilidad] = @DisponibilidadNEW ,
                        [Cotizado] = 1, [SubTotal] = @Subtotal, [FechaVigencia] = @FechaVigencia ,
                        [ModificadoProveedorPor] = @IdProveedorActual, [IdMaterialVendedor] = @IdMaterialVendedor ,
                        [NoCotizar] = @NoCotizar, [IdUnidadProveedor] = @IdUnidadVendedor ,
                        [UnidadProveedor] = @UnidadCotizada, [MaterialCotizadoTextoC] = @MaterialCotizadoTextC ,
         ...
         -----------------------------------------------------------------------------------------------------
                IF ISNULL ( @NoCotizar_Actual, 0 ) <> @NoCotizar
                    BEGIN
                        ...
                        SET @Detalle
                            = ( @Detalle + ' *Disponibilidad a cotizar de: '
                        ...
                        SET @Detalle = ( @Detalle + ' *Moneda: ' + ISNULL ( @MONEDA, '' )) ;
                        SET @Detalle = ( @Detalle + ' *Comentario: ' + ISNULL ( @ComentarioSubcontratista, '' )) ;
                        SET @Detalle
                            = ( @Detalle + ' *Fecha vigencia cotización de: '
                                + CAST(ISNULL ( @FechaVigencia, '' ) AS NVARCHAR (MAX))) ;
                        SET @Detalle
                            = ( @Detalle + ' *Fecha entrega de cotización: '
                ...
                ELSE
                    BEGIN
                        ...
                        SELECT  @IdMaterialVendedor_Actual = IdMaterialVendedor ,
                                @PrecioUnitario_Actual = PrecioUnitario, @Disponibilidad_Actual = Disponibilidad ,
                                @IdMoneda_Actual = IdMoneda ,
                                @ComentarioSubcontratista_Actual = ComentarioSubcontratista ,
                                @FechaVigencia_Actual = FechaVigencia, @FechaEntrega_Actual = FechaEntrega,
                                @IdCondicionDePago_Actual=IdCondicionPago, @DiasCredito_Actual=DiasCredito
                          FROM  dbo.MM_PeticionOfertaDetalle
                         WHERE  IdPeticionOfertaDetalle = @IdPeticionOfertaDetalle
                                ...
                                SET @Detalle
                                    = ( @Detalle + ' *Cambio material a cotizar de: '
                                        ...
                                        +   (SELECT CONCAT (DescripcionCorta,
                                                 ...
                                                 ---DescripcionCorta
                                                FROM    dbo.MM_Material
                                               WHERE    IdMaterial = @IdMaterialVendedor_Actual ) + ' a: '
                                        + CAST(ISNULL ( @IdMaterialVendedor, 0 ) AS NVARCHAR (MAX)) + ' - '
                                        + (   SELECT    CONCAT (DescripcionCorta,
                                                 ' Marca: ', CASE WHEN ISNULL(LEN(Marca),0)>0 THEN Marca ELSE ' S/M' END,
                                                 ' Modelo: ', CASE WHEN ISNULL(LEN(Modelo),0)>0 THEN Modelo ELSE ' S/M' END,
                        ...
                        IF ISNULL ( @IdMoneda_Actual, 0 ) <> @IdMoneda
                            BEGIN
                                ...
                                SET @MONEDA = ( SELECT TipoMonedaCorto FROM  dbo.PV_TipoMoneda WHERE
                                                        IdMoneda = @IdMoneda                ) ;
                                SET @Detalle
                                    = ( @Detalle + ' *Cambio moneda de: ' + ISNULL ( @MONEDA_ACTUAL, '' ) + ' a: '
                                        + ISNULL ( @MONEDA, '' ) + ' ' ) ;
                            END ;
                        IF ISNULL ( @ComentarioSubcontratista_Actual, '' ) <> @ComentarioSubcontratista
                            BEGIN
                                SET @Detalle
                                    = ( @Detalle + ' *Cambio comentario de: '
                        ...
                        IF @FechaEntrega_Actual <> @FechaEntrega
                            BEGIN
                                SET @Detalle
                                    = ( @Detalle + ' *Cambio fecha entrega cotización de: '
                ...
                IF @NoCotizar_Actual <> @NoCotizar
                    BEGIN
                        SELECT  @IdMaterialVendedor_Actual = IdMaterialVendedor ,
                                ...
                                @ComentarioSubcontratista_Actual = ComentarioSubcontratista ,
                                @FechaVigencia_Actual = FechaVigencia, @FechaEntrega_Actual = FechaEntrega,
                                @IdCondicionDePago_Actual=IdCondicionPago, @DiasCredito=DiasCredito
                          FROM  dbo.MM_PeticionOfertaDetalle
                         WHERE  IdPeticionOfertaDetalle = @IdPeticionOfertaDetalle
                        SET @Detalle
                            = ( @Detalle + ' *Se cambio de Cotizar a No Cotizar servicio/material. Datos anteriores: ' ) ;
                        SET @Detalle
                        ...
                        SET @Detalle
                            = ( @Detalle + ' *Comentario de: ' + ISNULL ( @ComentarioSubcontratista_Actual, '' ) + ' ' ) ;
                        SET @Detalle
                            = ( @Detalle + ' *Fecha vigencia cotización '
                                + CAST(ISNULL ( @FechaVigencia_Actual, '' ) AS NVARCHAR (MAX)) + ' ' ) ;
                        SET @Detalle
                            = ( @Detalle + ' *Fecha entrega cotización '
