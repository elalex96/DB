USE [Petrovendor]
GO
DROP PROCEDURE IF EXISTS SEL_CO_ObtenControlPresupuestalContrato
/****** Object:  StoredProcedure [dbo].[SEL_CO_ObtenControlPresupuestalContrato]    Script Date: 04/06/2025 09:44:54 a. m. ******/
SET ANSI_NULLS ON
GO
...
-- Description: Consulta de los pedidos y facturas asocidados al presupuesto
-- =============================================
CREATE PROC [dbo].[SEL_CO_ObtenControlPresupuestalContrato] --10036,10183
@IdContrato int,
@IdPresupuesto int,
@IdUsuario int = null
AS
BEGIN
	...
	DECLARE @Presupuesto NVARCHAR(MAX) = (SELECT Nombre
											FROM Adinco.dbo.CO_Presupuesto
											WHERE IdPresupuesto = @IdPresupuesto
											group by Nombre);
	DROP TABLE IF EXISTS #TablaRequisicion
	CREATE TABLE #TablaRequisicion
	(
	...
	DROP TABLE IF EXISTS #AceptacionPedido
	CREATE TABLE #AceptacionPedido
	(
		IdPedido INT,
	...
	DROP TABLE IF EXISTS #CantidadAceptadaMontos
	CREATE TABLE #CantidadAceptadaMontos
	(
		IdAceptacionPedido INT,
		IdAceptacionPedidoDetalle INT,
		IdPedidoDetalle INT,
	...
	DROP TABLE IF EXISTS #LineasPresupuesto
	CREATE TABLE #LineasPresupuesto
	(
		IdLineaPresupuesto INT, 
		NombreLinea NVARCHAR(MAX)
	)
	
	DROP TABLE IF EXISTS #AceptacionFactura
	...
	FROM #TablaRequisicion req
		LEFT JOIN #CantidadAceptadaMontos montoAcept
			ON req.IdPedidoDetalle = montoAcept.IdPedidoDetalle
		LEFT JOIN #AceptacionPedido ap
			ON montoAcept.IdAceptacionPedido = ap.IdAceptacionPedido
			   AND montoAcept.IdAceptacionPedidoDetalle = ap.IdAceptacionPedidoDetalle
