CREATE VIEW [dbo].[EstatusPedidos]
AS

	SELECT PG.IdPedido AS Pedido,
       P.IdSolicitudPedido AS SolicitudPedido,
...
/*SI EXISTE UN REGISTRO DE ACEPTACIÓN DE PEDIDO YA DEBE ESTAR CONFIRMADO EL PEDIDO*/
           WHEN AP.IdAceptacionPedido IS NULL
           THEN 'No se ha realizado ninguna aceptación de servicio'
           ELSE CAST(AP.IdAceptacionPedido AS NVARCHAR(MAX))
       END AS AceptacionPedido,
       CASE
           WHEN EAF.IdEstatus IS NULL
           THEN 'No se ha realizado ninguna aprobación de factura'
           ELSE EAF.Nombre
       END AS EstatusFactura,
       CASE
           WHEN EAF.IdEstatus IS NULL
                AND TRANS.PDF IS NULL
           THEN 'No se ha realizado ningún pago'
           WHEN EAF.IdEstatus IS NOT NULL
                AND TRANS.PDF IS NULL
           THEN 'Pendiente de pago'
           WHEN EAF.IdEstatus IS NOT NULL
                AND TRANS.PDF IS NOT NULL
           THEN 'Pago realizado'
           ELSE 'No se ha realizado ningún pago'
       END AS EstatusPago,
      CONCAT(FA. Serie, ' ', fa.Folio) AS Factura ,
       CASE
...
/*SI LA APROBACIÓN DE PEDIDO ES RECHAZADA(3) NUNCA SE ENVIA LA CONFIRMACIÓN AL PROVEEDOR*/
       ...
       CASE
           WHEN AP.IdAceptacionPedido IS NULL
           THEN 'No se ha solicitado formato de carta de contenido nacional al proveedor'
           ELSE 
/*BUSCAR LA ULTIMA APROBACIÓN DE CARTA DE CONTENIDO NACIONAL YA QUE ESA ES LA QUE REALMENTE PROVOCA LA SOLICITUD DE APROBACIÓN DE FACTURA*/
       ISNULL(
		(
			SELECT TOP 1 ECCN.Nombre
			FROM dbo.TA_Estatus ECCN
				 INNER JOIN dbo.MM_AceptacionCartaPCN CCN ON CCN.IdEstatus = ECCN.IdEstatus
			WHERE CCN.IdAceptacionPedido = AP.IdAceptacionPedido
			ORDER BY CCN.CreadoEl DESC
		), 'No se ha recibido solicitud de aprobación de Carta de Contenido Nacional por parte del proveedor')
       END AS EstatusCN,
       TRFAC.CreadoEn AS FechaRegistroTranferencia,
	   TRANS.MontoPagado	AS [Monto],
	   MTRANS.TipoMonedaCorto	AS [MonedaTransfer],
	   dbo.fnGetMontoPedido(P.IdPedido)	AS [MontoPedido],
	   dbo.fnGetInstalacion(P.IdSolicitudPedido)	AS [Instalación]
FROM MM_Pedido AS P
     ...
     LEFT JOIN dbo.TA_Operacion APF ON APF.IdDocumento = AFF.IdAceptacionFactura
                                       AND APF.IdTipoOperacion = 10 -->Aprobación Factura 
     LEFT JOIN dbo.TA_Estatus EAF ON EAF.IdEstatus = APF.IdEstatusOperacion
     LEFT JOIN dbo.FI_Factura AS FP ON FP.IdFactura = AFF.IdFactura
                                       AND FP.Activa = 1
	 ...
	 LEFT JOIN Adinco.dbo.PV_TipoMoneda	MTRANS ON TRANS.IdMoneda	=	MTRANS.IdMoneda
WHERE O.IdTipoOperacion = 9
      AND O.IdProveedor IN(606, 690,1835)
...
GROUP BY P.IdPedido,
         ...
         EAF.Nombre,
         TRANS.PDF,
        CONCAT(FA. Serie, ' ', fa.Folio),
         Ta.IdTarea,
         HV.FechaVigencia,
         EAF.IdEstatus,
         TRFAC.CreadoEn,
		 TRANS.MontoPagado,
	     MTRANS.TipoMonedaCorto
