-- select dbo.fnCantidadConLetraMoneda(53.60,'Dolares')
create FUNCTION [dbo].[fnCantidadConLetraMoneda]
(
	@Numero DECIMAL(18, 2),
...
AS
     BEGIN
         ...
         DECLARE @lnEntero INT, @lcRetorno VARCHAR(512), @lnTerna INT, @lcMiles VARCHAR(512), @lcCadena VARCHAR(512), @lnUnidades INT, @lnDecenas INT, @lnCentenas INT, @lnFraccion INT;
         ...
         SELECT @ImpLetra = RTRIM(@lcRetorno)+' ' +upper(@nombreMoneda)+'  -  '+SUBSTRING(@sFraccion, LEN(@sFraccion)-1, 2)+'/100';
         RETURN @ImpLetra ;
     END;