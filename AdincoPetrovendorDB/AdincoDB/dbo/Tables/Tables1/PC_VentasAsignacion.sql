...
CREATE NONCLUSTERED INDEX [idx_PuntoVenta]
    ON [dbo].[PC_VentasAsignacion]([PUNTO DE VENTA] ASC) WITH (STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [idx_CampoOficial]
    ON [dbo].[PC_VentasAsignacion]([CAMPO OFICIAL] ASC) WITH (STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [idx_ProductoAgrupa]
    ON [dbo].[PC_VentasAsignacion]([PRODUCTO AGRUPA] ASC) WITH (STATISTICS_NORECOMPUTE = ON);
