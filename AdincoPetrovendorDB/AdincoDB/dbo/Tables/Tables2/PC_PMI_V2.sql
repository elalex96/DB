CREATE TABLE [dbo].[PC_PMI_V2] (
    [IdPMI]           INT            IDENTITY (10000, 1) NOT NULL,
    [IdContrato]      INT            NULL,
    ...
    CONSTRAINT [PK_PC_PMI_V2] PRIMARY KEY CLUSTERED ([IdPMI] ASC) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON),
    CONSTRAINT [FK_PC_PMI_V2_CO_Contrato] FOREIGN KEY ([IdContrato]) REFERENCES [dbo].[CO_Contrato] ([IdContrato])
);


GO
CREATE NONCLUSTERED INDEX [idx_UUID]
    ON [dbo].[PC_PMI_V2]([UUID] ASC) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON);
