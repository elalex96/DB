CREATE VIEW [dbo].[VistaFacturasIHSA]
AS
SELECT DISTINCT
    CON.NumeroContrato,
    ...
    CASE
        WHEN TF.IdTransfer IS NULL THEN
            TTFCP.IdTransferencia
        WHEN TF.IdTransfer IS NOT NULL THEN
            T.IdTransferencia
    END AS IdTransferencia,
    CASE
        WHEN TF.IdTransfer IS NULL THEN
            TTFCP.ReferenciaBancaria
        WHEN TF.IdTransfer IS NOT NULL THEN
    ...
    CASE
        WHEN TF.IdTransfer IS NULL THEN
            TMTT.TipoMonedaCorto
        WHEN TF.IdTransfer IS NOT NULL THEN
            TMT.TipoMonedaCorto
    END AS MonedaPago,
    CASE
        WHEN TF.IdTransfer IS NULL THEN
            CAST(TTFCP.MontoPagado AS FLOAT)
        WHEN TF.IdTransfer IS NOT NULL THEN
            CAST(T.MontoPagado AS FLOAT)
    END AS MontoPagadoTransferencia,
    CASE
        WHEN TF.IdTransfer IS NULL THEN
            CAST(ISNULL(TTFCP.MontoPagado, 0) / TCDTT.TipoCambio AS FLOAT)
        WHEN TF.IdTransfer IS NOT NULL THEN
    ...
    (CASE
         WHEN ISNULL(R.MontoRegistro, 0) <> 0 THEN
             CAST(ISNULL(R.MontoRegistro, 0) / TCDF.TipoCambio AS FLOAT)
         ELSE
             0
     END
    )
...
FROM dbo.FI_Factura F (NOLOCK)
    JOIN dbo.PV_Subcontratista SE (NOLOCK)
        ON F.IdSubcontratista = SE.IdSubcontratista
           AND F.IdContrato IN ( 10031, 10034, 10035 )
    JOIN dbo.PV_Subcontratista SR (NOLOCK)
        ON F.Receptor = SR.RFC
    JOIN dbo.CO_Contrato CON (NOLOCK)
        ON F.IdContrato = CON.IdContrato
    LEFT JOIN dbo.FI_TransferFactura TF (NOLOCK)
        ON F.IdFactura = TF.IdFactura
    LEFT JOIN dbo.FI_Transfer T (NOLOCK)
        ON TF.IdTransfer = T.IdTransferencia
           AND T.IdContrato IN ( 10031, 10034, 10035 )
    LEFT JOIN dbo.CO_Registro R (NOLOCK)
        ON F.IdFactura = R.IdFactura
    LEFT JOIN dbo.CO_LineaPresupuestoMes L (NOLOCK)
        ON R.IdPrograma = L.IdLineaPresupuestoMes
    LEFT JOIN dbo.CO_GastosRubro GR (NOLOCK)
    ...
    LEFT JOIN dbo.CO_SubactividadPetrolera SP (NOLOCK)
        ON L.IdSubactividadPetrolera = SP.IdSubactividadPetrolera
    LEFT JOIN dbo.CO_TareaPetrolera TA (NOLOCK)
    ...
    LEFT JOIN dbo.PV_TipoMoneda TMT (NOLOCK)
    ...
    LEFT JOIN dbo.CO_TipoCambioDiario TCDT (NOLOCK)
    ...
    LEFT JOIN dbo.FI_CPDocRelacionado FCPDR (NOLOCK)
    ...
    LEFT JOIN dbo.FI_Factura FCP
        ON CP.IdFactura = FCP.IdFactura
    LEFT JOIN dbo.FI_TransferFactura TFCP (NOLOCK)
        ON CP.IdFactura = TFCP.IdFactura
    LEFT JOIN dbo.FI_Transfer TTFCP (NOLOCK)
        ON TFCP.IdTransfer = TTFCP.IdTransferencia
           AND TTFCP.IdContrato IN ( 10031, 10034, 10035 )
    LEFT JOIN dbo.PV_TipoMoneda TMTT (NOLOCK)
        ON TMTT.IdMoneda = TTFCP.IdMoneda
    LEFT JOIN dbo.CO_TipoCambioDiario TCDTT (NOLOCK)
