CREATE VIEW [dbo].ConsultaGastosJaguarPantera_Finanzas
AS
     SELECT C.NumeroContrato,
			...
			R.PCN						AS [PCN],
			ISNULL(F.MontoConIva,'')	AS [MontoFactura],
			ISNULL(F.Moneda,'')			AS [MonedaFactura],
 			dbo.fn_AprobacionFactura(F.UUID)	AS [FecAprobFactura],
            CASE
                WHEN R.CvTipoDocFacturacion = 1
                THEN SF.RFC
                WHEN R.CvTipoDocFacturacion IN(2, 3)
	...
	LEFT JOIN dbo.CO_AreaContractual ACC (NOLOCK)
		ON ACC.IdAreaContractual = C.IdAreaContractual
	LEFT JOIN dbo.FI_TransferFactura	TF
		ON F.IdFactura	=	TF.IdFactura
	LEFT JOIN dbo.FI_Transfer	TR
	...
	--ISNULL(TR.MontoPagado,0),
		...
		ISNULL(F.Moneda,''),
		dbo.fn_AprobacionFactura(F.UUID),
		CASE
			WHEN R.CvTipoDocFacturacion = 1
			THEN SF.RFC
			WHEN R.CvTipoDocFacturacion IN(2, 3)
