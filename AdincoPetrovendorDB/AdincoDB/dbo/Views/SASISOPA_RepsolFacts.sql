CREATE VIEW dbo.SASISOPA_RepsolFacts
AS
...
--	dbo.fnGetCantidadInstancias(CE.IdContratoEntregable)	AS Cantidad,
	100.00/dbo.fnGetCantidadAccionesXPolitica(PIP.IdProgramaImplementaPolitica)	AS	PorcentajePolitica,
	...
	ISNULL(CE.FocalPoint,'')	AS	ElaboradorInternoEmail,
	...
	N.Cardinal AS Orden,
	dbo.fnGetAvanceXPolitica(PIP.IdProgramaImplementaPolitica)	AS AvanceRealPolitica
FROM
	CO_ProgramaImplementa	CPI	(NOLOCK)
JOIN
	CO_Contrato	C	(NOLOCK)
	ON	CPI.IdContrato	=	C.IdContrato
	AND	C.IdContratista = 10050
JOIN
...
GROUP BY
	...
	END, 
	PID.Descripcion, 
	dbo.fnGetCantidadInstancias(CE.IdContratoEntregable),
	PIA.Porcentaje,
	dbo.fnGetCantidadAccionesXPolitica(PIP.IdProgramaImplementaPolitica),
	CASE WHEN AACT.EstadoID = 10000 THEN 0
		WHEN AACT.EstadoID = 10001 THEN 0
		WHEN AACT.EstadoID = 10002 THEN 0
		WHEN AACT.EstadoID IS NULL THEN 0
	...
	N.Cardinal,
	dbo.fnGetAvanceXPolitica(PIP.IdProgramaImplementaPolitica)
