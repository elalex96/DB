IF EXISTS (
    SELECT 1
    FROM sys.views
    WHERE object_id = OBJECT_ID(N'[dbo].[GastosAmatitlan2020]')
)
BEGIN
    DROP VIEW [dbo].[GastosAmatitlan2020];
END
GO
...
AS
    ...
    SELECT
                                    ...
                                    THEN (CASE
                                              WHEN (F.TipoComprobante) LIKE '%egreso%'
                                                  ...
                                                  THEN ROUND(
                                                                ...
                                                                / ISNULL(RM.TipoCambio, TCDF.TipoCambio), 2
                                                            )
                                              ELSE
                                                  ROUND(
        ...
        CASE
            WHEN RM.MesEstadoPemex IS NULL
                THEN ''
            WHEN CAST(MONTH(RM.MesEstadoPemex) AS INT) < 10
                THEN CAST(YEAR(RM.MesEstadoPemex) AS VARCHAR(10)) + ' - 0'
        ...
        SUM(   CASE
                   WHEN ISNULL(RM.ImporteEstimadoParcial, 0) <> 0
                       THEN CASE
                                WHEN ISNULL(CO_EstadoRegistro_V2.NombreEstado, 'Revisión') LIKE '%Aprobado%'
                                    THEN CAST(ISNULL(RM.ImporteEstimadoParcial, 0)
                                              / (CASE
                                                     WHEN R.CvTipoDocFacturacion = 1
                                                         THEN ISNULL(RM.TipoCambio, TCDF.TipoCambio)
                                                     WHEN R.CvTipoDocFacturacion IN (
    ...
    FROM
        ...
        JOIN
            ...
            ON CO_AnioContractual.IdAnioContractual =   P.IdAnioContractual
            AND IdContrato = 10007  -- CONTRATO AMATITLAN, TOMA EN CUENTA TODOS LOS PRESUPUESTOS
        JOIN
