...
/****** Object:  View [dbo].[GastosAmatitlan2020]    Script Date: 10/11/2021 09:19:31 a. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[GastosAmatitlan2020]
AS
SELECT        R.IdRegistro,
			  ...
			  CASE WHEN R.CvTipoDocFacturacion = 1 THEN SUM(CASE WHEN ISNULL(R.MontoRegistro+ RM.MontoEquivalente, 0) <> 0 THEN ROUND(ISNULL(R.MontoRegistro+ RM.MontoEquivalente, 0) / TCDF.TipoCambio, 2) ELSE 0 END) WHEN R.CvTipoDocFacturacion IN (2, 3) 
                THEN SUM(CASE WHEN ISNULL(R.MontoRegistro + RM.MontoEquivalente, 0) <> 0 THEN ROUND(ISNULL(R.MontoRegistro+ RM.MontoEquivalente, 0) / TCDPC.TipoCambio, 2) ELSE 0 END) END AS MontoUSDConMarkup,
              CASE WHEN R.CvTipoDocFacturacion = 1 THEN SF.RazonSocial WHEN R.CvTipoDocFacturacion IN (2, 3) THEN SPC.RazonSocial END AS Subcontratista, 
			  IR.NombreInstalacion AS InstalacionRegistro, 
			  R.InicioEjecucion, 
              R.FinEjecucion, 
			  U.Nombre AS CreadoPor, 
			  R.MontoRegistro, 
			  CASE WHEN R.CvTipoDocFacturacion = 1 THEN TMF.TipoMonedaCorto WHEN R.CvTipoDocFacturacion IN (2, 3) THEN TMPC.TipoMonedaCorto END AS Moneda, 
              R.MesPresentacion, 
			  ...
			  CASE WHEN R.CvTipoDocFacturacion = 1 THEN F.IdFactura WHEN R.CvTipoDocFacturacion IN (2, 3) 
				THEN PC.IdPedimentoComprobante END AS Identificador, 
			  LPM.IdLineaPresupuestoMes AS LineaPresupuesto, 
			  P.Nombre AS Presupuesto, 
			  CASE WHEN R.CvTipoDocFacturacion = 1 THEN TCDF.TipoCambio WHEN R.CvTipoDocFacturacion in (2, 3) THEN TCDPC.TipoCambio END AS TipoCambio, ------TipoCambio CF, PI, PE
...
FROM            dbo.CO_LineaPresupuestoMes AS LPM WITH (NOLOCK) LEFT OUTER JOIN
                         ...
                         LEFT JOIN CO_RegistroMarkup RM ON R.IdRegistro = RM.GastoId  --21/10/2021 DR
WHERE        (P.IdPresupuesto IN (10048, 10037, 10065, 10075, 10159, 10188, 10038, 10000, 10169, 10089)) AND (R.IdRegistro IS NOT NULL)
GROUP BY PC.NumeroPedimento, S.NombreServicio, I.NombreInstalacion, LPM.AC_FEC_INI, LPM.AC_FEC_FIN, F.Fecha, LTRIM(RTRIM(ISNULL(F.Serie, '') + ' ' + ISNULL(F.Folio, ''))), SF.RazonSocial, IR.NombreInstalacion, 
                         ...
                         R.IdRegistro, PC.FolioComprobante, SPC.RazonSocial, TMPC.TipoMonedaCorto, CASE WHEN R.CvTipoDocFacturacion = 1 THEN TCDF.TipoCambio WHEN R.CvTipoDocFacturacion in (2, 3) THEN TCDPC.TipoCambio END, ISNULL(F.UUID, ''), RM.Porcentaje
GO
