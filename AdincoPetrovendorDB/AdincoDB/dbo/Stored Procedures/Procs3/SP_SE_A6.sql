IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_SE_A6]') AND type IN (N'P'))
    DROP PROCEDURE [dbo].[SP_SE_A6];
GO
-- =============================================
-- Author:		Manuel Cruz
...
BEGIN
	...
	SELECT @RazonSocial = CA.RazonSocial
		...
		WHERE C.IdContrato	=	@IdContrato;
		 ...
		 BEGIN
			EXEC [SP_SE_A6_MPY]@IdContrato,@IdUsuario,@IdPresupuesto,@FInicio,@FFin,@IdPeriodo,@Etapa;
		 END
		 ELSE
		 BEGIN
