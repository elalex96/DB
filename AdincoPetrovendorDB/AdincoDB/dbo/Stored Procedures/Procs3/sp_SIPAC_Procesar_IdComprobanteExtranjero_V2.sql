CREATE PROCEDURE [dbo].[sp_SIPAC_Procesar_IdComprobanteExtranjero_V2]
...
AS
     BEGIN
         ...
         UPDATE dbo.FI_PedimentoComprobante
           SET 
               IdDocFacturacionSIPAC = CONCAT('PE', '-', LEFT('0'+CAST(MONTH(@Mes) AS VARCHAR(2)), 2)+CAST(YEAR(@Mes) AS CHAR(4)), '-', RIGHT('000000'+CAST(PCT.SIPAC AS VARCHAR(6)), 6))
         ...
         --EXEC sp_SIPAC_Procesar_IdComprobanteExtranjero_V2 3,'2017-03-01',3
