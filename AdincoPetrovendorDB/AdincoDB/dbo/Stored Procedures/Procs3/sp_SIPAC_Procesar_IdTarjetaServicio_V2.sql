CREATE PROCEDURE [dbo].[sp_SIPAC_Procesar_IdTarjetaServicio_V2]
...
AS
     BEGIN
         ...
         UPDATE dbo.FI_Transfer
           SET 
               IdComprobantePago = CONCAT('TS', '-', LEFT('0'+CAST(MONTH(@Mes) AS VARCHAR(2)), 2)+CAST(YEAR(@Mes) AS CHAR(4)), '-', RIGHT('000000'+CAST(TRT.SIPAC AS VARCHAR(6)), 6))
         FROM FI_Transfer TR
              JOIN #FI_Transfer TRT ON TR.IdTransferencia = TRT.IdTransferencia
         WHERE TR.IdTransferencia = TRT.IdTransferencia;
         --EXEC sp_SIPAC_Procesar_IdTarjetaServicio_V2 3,'2017-03-01',3
