...
(
    SELECT 1
    FROM dbo.sysobjects
    WHERE name = 'USD_SEL_FI_MPY_ConsultaFacturasPorContratoDescarga'
)
    DROP PROCEDURE USD_SEL_FI_MPY_ConsultaFacturasPorContratoDescarga
GO
-- =============================================  
-- Author: DANIEL AC
-- Create date: 02-10-2025  
-- Description: Lista las facturas de un contrato MURPHY (10053,10039)  --> CTES PARA CONTRATOS DE MURPHY CUENCA SALINA Y EL DORADO
-- =============================================  
CREATE PROCEDURE [dbo].[USD_SEL_FI_MPY_ConsultaFacturasPorContratoDescarga]
...
BEGIN
    ...
    FROM Petrovendor.dbo.MM_AceptacionCartaPCN AS AC WITH (NOLOCK)
        ...
        JOIN Petrovendor.dbo.MM_Pedido AS P WITH (NOLOCK)
            ON P.IdContrato IN (10053,10039)  --> CTES PARA CONTRATOS DE MURPHY CUENCA SALINA Y EL DORADO
        ...
        FROM dbo.FI_Factura AS F (NOLOCK)
            JOIN dbo.PV_Subcontratista AS S WITH (NOLOCK)
                ON F.IdContrato  IN  (10053,10039)  --> CTES PARA CONTRATOS DE MURPHY CUENCA SALINA Y EL DORADO
		...
		-- FACTURAS QUE NO TIENEN EL CONTRATO PERO SON DE MURPHY 
		 INSERT INTO #Facturas
        (
            IdFactura,
            NombreEmisor,
        ...
        FROM dbo.FI_Factura AS F (NOLOCK)
            JOIN dbo.PV_Subcontratista AS S WITH (NOLOCK)
                ON ISNULL(F.IdContrato,0) = 0
				   AND F.Receptor in ('MSU150922EYA','DEP180824FS8') --> CTES PARA CONTRATOS DE MURPHY CUENCA SALINA Y EL DORADO
                   AND CONVERT(DATE, ISNULL(F.FechaTimbrado, F.Fecha))
                   BETWEEN CONVERT(DATE, @FechaInicio) AND CONVERT(DATE, @FechaFin)
                   AND F.IdSubcontratista = S.IdSubcontratista
            LEFT JOIN dbo.CO_Contrato C WITH (NOLOCK)
                ON F.IdContrato = C.IdContrato
            LEFT JOIN dbo.CO_Contratista CC WITH (NOLOCK)
                ON C.IdContratista = CC.IdContratista
                   AND F.Receptor <> CC.RFC 
        ...
        FROM dbo.FI_FacturaContrato FC WITH (NOLOCK)
            JOIN dbo.FI_Factura AS F WITH (NOLOCK)
				...
				AND	FC.IdContrato IN  (10053,10039)  --> CTES PARA CONTRATOS DE MURPHY CUENCA SALINA Y EL DORADO
                   AND CONVERT(DATE, ISNULL(F.FechaTimbrado, F.Fecha))
                   BETWEEN CONVERT(DATE, @FechaInicio) AND CONVERT(DATE, @FechaFin)
          JOIN dbo.PV_Subcontratista AS S WITH (NOLOCK)
    ...
    FROM #Facturas TEMP
        JOIN dbo.FI_Documento D (NOLOCK)
            ON TEMP.IdFactura = D.IdFactura
               ...
               AND D.IdTipoDocumento = 1 --> Factura
  ...
  FROM #Facturas F
	...
	,'/'
	,(CASE WHEN LEN(NombreEmisor)>40 THEN 
		REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(RTRIM(LTRIM(LEFT(NombreEmisor,40)))), '/', ''), ':', ''), '*', ''), '"', ''), '<', ''), '>', ''), '|', ''), '\', '')
	ELSE
		REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(RTRIM(LTRIM(NombreEmisor))), '/', ''), ':', ''), '*', ''), '"', ''), '<', ''), '>', ''), '|', ''), '\', '')
	END))

	-- BUSCAR EL ID DE PETROVENDOR POR SI EL PDF O XML NO ESTA EN ADINCO
	UPDATE  FT
	SET FT.IdFacturaPetrovendorTemp = FP.IdFactura
	FROM #Facturas FT
	JOIN Petrovendor..FI_Factura FP
		ON UPPER(FT.UUID)  COLLATE Modern_Spanish_CI_AS = UPPER(FP.UUID)  COLLATE Modern_Spanish_CI_AS
	

    SELECT  ROW_NUMBER() OVER (
			ORDER BY F.IdFactura DESC) AS IdRow,
		   F.IdFactura,
	       F.RFC_Receptor AS RFC_Receptor,
		   ISNULL(F.IdFacturaPetrovendorTemp,0) AS IdFacturaPetrovendor,
		   F.RutaCarpeta,
		   ISNULL(F.IdContrato,0) as IdContrato,
           ...
           UPPER(ISNULL(F.UUID, 'NA')) AS UUID,
           F.FechaTimbrado,
    ...
    FROM #Facturas F
	WHERE F.TipoComprobante = 'I'
	AND F.RFC_Receptor in ('MSU150922EYA','DEP180824FS8')   --> CTES PARA RECEPTORES SOLO DE EMPRESA DE  MURPHY CUENCA SALINA Y EL DORADO
