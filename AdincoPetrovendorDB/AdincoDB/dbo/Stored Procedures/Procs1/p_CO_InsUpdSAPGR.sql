...
-- Ajuste:		Se agrega el GRReferenceNumber dentro de la 
...
(
    select 1
    from [CO_SAPGR]
    where PO_SAPNumber = @pPO_SAPNumber
          and PO_SAPNumber = @pPO_SAPNumber
          and POLineNumber = @pPOLineNumber
          and DocumentDate = @pDocumentDate
		  and GRReferenceNumber = @pGRReferenceNumber
)
begin
...
Begin
    ...
    set Quantity = @pQuantity,
        ...
        UOM = @pUOM,
        Plant = @pPlant,
        GRReferenceNumber = case
                                when isnull(GRReferenceNumber, '') = '' then
                                    @pGRReferenceNumber
                                else
                                    GRReferenceNumber
                            end,
