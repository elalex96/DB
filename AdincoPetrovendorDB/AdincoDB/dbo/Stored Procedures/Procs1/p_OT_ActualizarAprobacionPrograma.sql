...
AS  
 ...
 BEGIN  
  ...
  IF EXISTS (
    ...
    convert(varchar,sp.Fecha,112) BETWEEN convert(VARCHAR,@fechaIni,112) 
	AND convert(VARCHAR,@fechaFin,112)   
	AND
	CASE WHEN DATEPART(WEEKDAY,Fecha) = 1 THEN @pDomingoAprob   
   ...
   END = 1 AND ISNULL(VoBoSubContratista,0) = 0 
  )
  BEGIN
	RAISERROR (15600,-1,-1, '[ALERTA]:No es posible actualizar, no se pueden aprobar días sin VoBo por el Proveedor. Refresque la pantalla ');    
	RETURN  
  END
 ...
 BEGIN  
   ...
   IF EXISTS (
	...
	CASE WHEN DATEPART(WEEKDAY,Fecha) = 1 THEN @pDomingoAprob   
         ...
         WHEN DATEPART(WEEKDAY,Fecha) = 6 THEN @pViernesAprob  
         WHEN DATEPART(WEEKDAY,Fecha) = 7 THEN @pSabadoAprob  
   END = 0 AND ISNULL(VoBoContratista,0) = 1 
  )
  BEGIN
	RAISERROR (15600,-1,-1, '[ALERTA]:No es posible actualizar, no se puede desmarcar VoBo para días aprobados por Contratista. Refresque la pantalla ');    
	RETURN  
  END
