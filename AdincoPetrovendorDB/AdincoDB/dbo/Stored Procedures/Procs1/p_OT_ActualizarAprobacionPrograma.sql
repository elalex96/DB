IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'p_OT_ActualizarAprobacionPrograma'
    )
    DROP PROCEDURE p_OT_ActualizarAprobacionPrograma;
GO
CREATE PROCEDURE p_OT_ActualizarAprobacionPrograma
    @pIdOTSolicitud  INT,
    @pSemana         VARCHAR(21),
...
BEGIN
    ...
    IF exists
        ...
        BEGIN
            ...
            RAISERROR(
                         15600, -1, -1,
                         '[ALERTA]:Falta la captura de PR, la información no se guardó. Ya se notificó al area correspondiente '
                     );
            RETURN
        end
    ...
    IF (@pTipoUsuario = 1)
        BEGIN
            ...
            SET
                VoBoContratista = CASE
                                      WHEN DATEPART(WEEKDAY, Fecha) = 1
                                          THEN @pDomingoAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 2
                                          THEN @pLunesAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 3
                                          THEN @pMartesAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 4
                                          THEN @pMiercolesAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 5
                                          THEN @pJuevesAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 6
                                          THEN @pViernesAprob
                                      WHEN DATEPART(WEEKDAY, Fecha) = 7
                                          THEN @pSabadoAprob
                                      ELSE
                                          VoBoContratista
                                  END,
