...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[SP_CO_RptGastoPresupuestoXContrato] 
...
AS
     BEGIN
		 ...
		 SELECT
			DISTINCT p.IdPresupuesto
		FROM
			dbo.CO_AnioContractual	a
		JOIN
		...
		WHERE
         ...
         (IdActividad, 
          ...
          PresupuestoUSD
                ...
                WHERE(CO_LineaPresupuestoMes.IdPresupuesto IN (SELECT IdPresupuesto FROM #presupuestos))  --)= @IdPresupuesto)
   ...
   dbo.CO_ActividadPetroleraCNH.DescripcionActividadPetrolera, 
    ...
    LEFT JOIN Petrovendor.dbo.MM_AceptacionPedidoDetalle apd ON apd.IdPedidoDetalle = pd.IdPedidoDetalle
                ...
                WHERE lpm.IdPresupuesto IN (SELECT IdPresupuesto FROM #presupuestos) --= @IdPresupuesto
         ...
         SELECT 
				ISNULL(IdActividad, '') AS IdActividad, 
				...
				ISNULL(ActividadPetrolera, '') + '-' + ISNULL(SubactividadPetrolera, '') + '-' + ISNULL(TareaPetrolera, '') + '-' + ISNULL(SubTareaPetrolera, '') AS SubTareaPetrolera,
