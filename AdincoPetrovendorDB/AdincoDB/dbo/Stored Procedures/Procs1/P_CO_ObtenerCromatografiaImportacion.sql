...
CREATE PROC [dbo].[P_CO_ObtenerCromatografiaImportacion] @pIdContrato INT
AS
SELECT AreaContractual = ac.Descripcion,
...
INNER JOIN CO_CromatografiaValores cv ON cv.IdCromatografia = c.IdCromatografia
INNER JOIN CO_Contrato co ON co.IdContrato = c.IdContrato
INNER JOIN CO_AreaContractual ac ON ac.IdAreaContractual = co.IdAreaContractual
INNER JOIN [dbo].[CO_PuntosdeEntregaContrato] pec ON pec.[PuntoEntregaContratoID] = cv.IdPuntoEntregaContrato
INNER JOIN [dbo].[CO_PuntosdeEntrega] pe ON pe.PuntoEntregaID = pec.PuntoEntregaID
LEFT JOIN PR_ProduccionMensualSipac PMS ON c.IdContrato = PMS.idContrato
                                           AND pec.PuntoEntregaID = PMS.PuntoEntregaID
                                           AND DATEFROMPARTS(c.Anio, c.Mes, 1) = PMS.idFecha
                                           AND PMS.idHidrocarburo = 1001
LEFT JOIN PR_ProduccionMensualSipac PMSG ON c.IdContrato = PMSG.idContrato
