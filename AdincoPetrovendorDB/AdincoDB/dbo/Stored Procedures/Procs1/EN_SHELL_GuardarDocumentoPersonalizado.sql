...
BEGIN
     ...
     VALUES
           (
		    @IdPadre,
			@NombreArchivo,
			null,--CASE WHEN @EtapaId = 0 THEN NULL ELSE @EtapaId END,
			null,--CASE WHEN @ReceptorId = 0 THEN NULL ELSE @ReceptorId END,
			null,--CASE WHEN @InstalacionId = 0 THEN NULL ELSE @InstalacionId END, --> SI LA INSTALACION ES DIFERENTE DE CERO ENTONCES LA CARPETA ES GENERAL DE UN POZO
			null,--CASE WHEN @MarcoLegalId = 0 THEN NULL ELSE @MarcoLegalId END,
			null,--CASE WHEN @EtapaPozoId = 0 THEN NULL ELSE @EtapaPozoId END,
			null,
			@Frecuencia,
			@Frecuencia,
			'Archivo',
