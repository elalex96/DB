...
BEGIN
    ...
    EXEC [SIPAC_RC_CONT_26_M_IdDocFormaPago] @Contrato, @Mes, @IdPresupuesto;
    ...
    WHERE CO_Contrato.IdContrato = @Contrato
          ...
          AND CO_LineaPresupuestoMes.IdPresupuesto = CASE
                                                         ...
                                                         ELSE
                                                             @IdPresupuesto
                                                     END
    GROUP BY ISNULL(FI_Factura.TipoComprobanteEstandarizado, 'NA'),
    ...
    SELECT FI_Factura.IdFactura AS IdFacturaPUE,
           ...
           CAST((SUM(FI_TransferFactura.MontoPagado)) AS DECIMAL(15, 2)) AS MontoDivisaOriginal,
           FI_Factura.IdMoneda AS MonedaPUE,
           FI_Factura.MontoConIva,
           CAST((SUM(   CASE
                            WHEN FI_Transfer.IdMoneda = @DOLAR
                                 AND FI_Factura.IdMoneda = @DOLAR THEN
                                FI_TransferFactura.MontoPagado
                            WHEN ISNULL(CO_TipoCambioDiario.TipoCambio, 0) = 0 THEN
                                0
                            WHEN FI_Transfer.IdMoneda = @PESO
                                 AND FI_Factura.IdMoneda = @PESO THEN
    ...
    SELECT FI_Factura.IdFactura AS IdFacturaPUE,
           ...
           CAST((SUM(   CASE
                            ...
                            WHEN FI_Transfer.IdMoneda = @PESO
                                 AND FI_Factura.IdMoneda = @DOLAR THEN
                                FI_TransferFactura.MontoPagado / CO_TipoCambioDiario.TipoCambio
                            WHEN FI_Factura.IdMoneda NOT IN ( @PESO, @DOLAR ) THEN
                                FI_TransferFactura.MontoPagado / CO_TipoCambioDiario.TipoCambio
                        END
                    )
                ) AS DECIMAL(15, 2)) AS MontoDolares,
    ...
    SELECT CASE
           ...
           CASE
               WHEN CO_Registro.CvTipoDocFacturacion = @TipoFactura THEN
                   ISNULL(FI_Factura.UUID, 'NA')
           END AS [RC26_03],
           FI_Transfer.FechaPago AS [RC26_04],
           CASE
               WHEN FI_Transfer.AWSPDFId IS NULL THEN
                   'NOTA:Falta ingresar archivo PDF'
               ELSE
    ...
    FROM dbo.FI_Transfer WITH (NOLOCK)
        JOIN dbo.FI_TransferFactura WITH (NOLOCK)
            ON FI_Transfer.IdTransferencia = FI_TransferFactura.IdTransfer
               AND FI_Transfer.IdContrato = @Contrato
        JOIN #MontosTotalTransferenciaPUE MTT
        ...
        LEFT JOIN dbo.PV_Subcontratista PV_SubcontratistaNomina WITH (NOLOCK)
    ...
    FROM dbo.FI_Transfer WITH (NOLOCK)
        JOIN dbo.FI_TransferFactura WITH (NOLOCK)
            ON FI_Transfer.IdTransferencia = FI_TransferFactura.IdTransfer
               AND FI_Transfer.IdContrato = @Contrato
        JOIN #MontosTotalTransferenciaPPD MTT
    ...
    WHERE CO_Contrato.IdContrato = @Contrato
          ...
          AND CO_Servicio.NombreServicio NOT LIKE '%No elegibles%'
          AND FCP.UUID NOT IN (
                                  SELECT RPT.UUID FROM #uuidNoReportar RPT
                              )
          AND FCP.UUID NOT IN (
    ...
    SELECT CASE
           ...
           CASE
               WHEN CO_Registro.CvTipoDocFacturacion = @TipoPedimentoImportacion THEN
                   ISNULL(FI_PedimentoComprobante.NumeroPedimento, 'NA')
               WHEN CO_Registro.CvTipoDocFacturacion = @TipoComprobanteExtranjero THEN
                   ISNULL(FI_PedimentoComprobante.IdDocFacturacionSIPAC, 'NA')
           END AS [RC26_04],
           FI_Transfer.FechaPago AS [RC26_04],
           CASE
               WHEN FI_Transfer.AWSPDFId IS NULL THEN
           ...
           CAST((SUM(   CASE
                            ...
                            WHEN ISNULL(FI_TransferFactura.MontoPagado, 0) <> 0 THEN
                                CAST((ISNULL(FI_TransferFactura.MontoPagado, 0) / CO_TipoCambioDiario.TipoCambio) AS DECIMAL(15, 2))
                            ELSE
                                0
                        END
                    )
    ...
    FROM dbo.FI_Transfer WITH (NOLOCK)
        ...
        LEFT JOIN dbo.CO_TipoCambioDiario WITH (NOLOCK)
            ON CO_TipoCambioDiario.IdMoneda = FI_Transfer.IdMoneda
               ...
               AND MONTH(CO_TipoCambioDiario.Fecha) = MONTH(FI_Transfer.FechaPago)
               AND YEAR(CO_TipoCambioDiario.Fecha) = YEAR(FI_Transfer.FechaPago)
        LEFT JOIN dbo.PV_MetodoPago WITH (NOLOCK)
