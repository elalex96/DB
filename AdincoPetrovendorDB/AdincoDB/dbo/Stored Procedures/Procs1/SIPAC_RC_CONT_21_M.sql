         ...
         #Facturas.UUID,  
            ...
            AND FI_Factura.IdMoneda = 2  
                        ...
                        CASE  
                            ...
                            WHEN FI_Transfer.IdMoneda = 2  
                                 AND FI_Factura.IdMoneda = 1  
                                THEN CO_TipoCambioDiario.TipoCambio  
                        END                  AS TipoCambio,  
                        CO_TipoCambioDiario.Fecha,  
                        #Facturas.IdMoneda,  
                        FI_Transfer.IdMoneda AS MonedaTran,  
                    ...
                    (  
                        ...
                        RC21_28  
                           ...
                           SELECT DISTINCT  
                                ...
                                case           when len (CO_Presupuesto.IdPresupuestoCNH)>10            then             SUBSTRING(CO_Presupuesto.IdPresupuestoCNH, 22, 10)            else CO_Presupuesto.IdPresupuestoCNH END                     AS [RC21_0
0],  
    ...
    dbo.CO_Presupuesto WITH (NOLOCK)  
                        ...
                        RC21_28  
                            ...
                            SELECT  
                                ...
                                C.NumeroContrato                           AS [RF01_01],  
                                 case           when len (P.IdPresupuestoCNH)>10            then             SUBSTRING(P.IdPresupuestoCNH, 22, 10)            else P.IdPresupuestoCNH END                     AS [RC21_00],  
                                MONTH(CO_Registro.MesPresentacion)         AS [RC21_01],  
                                YEAR(CO_Registro.MesPresentacion)          AS [RC21_02],  
                                NULL                                       AS [RC21_03],  
                                ISNULL(SUBSTRING(F.IdDocFacturacionSIPAC, 1, 2),'CF') AS [RC21_04],-- SE AGREGA ISNULL CF, YA QUE SE ENCONTRO EN EL ISSUE 2394 NOTAS DE CREDITO EN PPD Y SOPORTE LAS AJUSTA A PUE PARA QUE SE PUEDAN PRESENTAR, ES CONSULTA SOLO DE FACTURA Y SE CONFIRMA QUE ES CORRECTO QUE NO SE VISUALICE EN LA HOJA 22 NI EN EL ZIP DE ARCHIVOS   AS [RC21_04],  
                                CASE  
                            ...
                            GROUP BY  
                                ...
                                C.NumeroContrato,  
                                 case           when len (P.IdPresupuestoCNH)>10            then             SUBSTRING(P.IdPresupuestoCNH, 22, 10)            else P.IdPresupuestoCNH END,  
     ...
     END,  
                        ...
                        LTRIM(RTRIM(C.IDRegFiducidiario))          AS [RI_00],  
                                ...
                                YEAR(CO_Registro.MesPresentacion)          AS [RC21_02],  
                                NULL                                       AS [RC21_03],  
                                 ISNULL(SUBSTRING(FCP.IdDocFacturacionSIPAC, 1, 2),'CF') AS [RC21_04],-- SE AGREGA ISNULL CF, YA QUE SE ENCONTRO EN EL ISSUE 2394 NOTAS DE CREDITO EN PPD Y SOPORTE LAS AJUSTA A PUE PARA QUE SE PUEDAN PRESENTAR, ES CONSULTA SOLO DE FACTURA Y SE CONFIRMA QUE ES CORRECTO QUE NO SE VISUALICE EN LA HOJA 22 NI EN EL ZIP DE ARCHIVOS
                                CASE  
                                    WHEN CO_Registro.CvTipoDocFacturacion = 1  
