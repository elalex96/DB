IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'sp_CO_ConsultaRegistrosGastos'
    )
            ...
            (
                ...
                IdFactura
                    ...
                    FROM
                        ...
                        LEFT JOIN
                            Petrovendor.dbo.MM_AceptacionFactura  AS AF (NOLOCK)
                                ON AP.IdAceptacionPedido = AF.IdAceptacionPedido
                        LEFT JOIN
                            Petrovendor.dbo.FI_Factura            AS FP (NOLOCK)
                                ON AF.IdFactura = FP.IdFactura
                                   AND FP.Activa = 1
                        LEFT JOIN
                            Adinco.dbo.FI_Factura                 AS FA (NOLOCK)
                    ...
                    SELECT
                        ...
                        CAST(CASE
                                 WHEN @NombreAreaContractual = 'Amatitlán'
                                     THEN @NombreEstadoInicialAmatitlan
                                 ELSE
                                     ER.NombreEstado
        ...
        IF (@NombreAreaContractual = 'Amatitlán')
            BEGIN
                UPDATE
                    #Datos
                SET
                    #Datos.EstadoValidacion = CO_EstadoRegistro_V2.NombreEstado
                FROM
                    #Datos
                    JOIN
                        CO_RegistroMarkup
