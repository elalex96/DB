CREATE PROCEDURE [dbo].[sp_CalculaMecanismoAjuste_V2]
    @IdContrato    INT,
    @Mes	  DATETIME
AS
BEGIN
...
(
    ...
    Total DECIMAL(16,3),
    Estado	 DECIMAL(16,3),
    Contratista	 DECIMAL(16,3),
    PorE  float,
    PorC  float
)

DECLARE
    @ValorContracHidro   DECIMAL(16,3),
    @IngresoAdicional   MONEY,
    @PorcRecuperacion   DECIMAL(5,3),
    @CostosPrevios	    MONEY,
    @CostosElegibles    MONEY,
    @CostosRecuperables MONEY,
    @LimRecuperacionCto MONEY,
    @CtoReconocidoRecuperado	  MONEY,
    @PrecioContracPetro MONEY,
    @PrecioContracConden MONEY,
    @PrecioContracGas MONEY,
    @ValorContracPetro MONEY,
    @ValorContracConden MONEY,
    @ValorContracGas MONEY,
    @RegaliaPetroleo    DECIMAL(5,3),
    @RegaliaCondensados    DECIMAL(5,3),
    @RegaliaGas  DECIMAL(5,3),
    @MontoRegaliaPetroleo    DECIMAL(16,3),
    @MontoRegaliaCondensados    MONEY,
    @MontoRegaliaGas  MONEY,
    @MontoTotalRegalias  DECIMAL(16,3),
    @UtilidadOperativa  MONEY,
    @U1 DECIMAL(5,3),
    @U2 DECIMAL(5,3),
    @SC1 DECIMAL(5,3),
    @SC2 DECIMAL(5,4),
    @MontoContraUtilidadOperEdo	  MONEY,
    @MontoContraUtilidadOperContratista	MONEY,
    @ValorActivos	MONEY,
    @ResulOperativoContratista  MONEY,
    @IndiceMetrica	DECIMAL(5,4),
    @MRO	  DECIMAL(7,4),
    @ProduccionPetroleo DECIMAL(16,3),
    @ProduccionCondesados   DECIMAL(16,3),
    @ProduccionGas	DECIMAL(16,3),
    @PagoRegaliasEdoPetroleo	  DECIMAL(16,3),
    @PagoRegaliasEdoCondensado  DECIMAL(16,3),
    @PagoRegaliasEdoGas	   DECIMAL(16,3),
    @PagoUtilidadOperEdoPetroleo	 DECIMAL(16,3),
    @PagoUtilidadOperEdoCondensado	 DECIMAL(16,3),
    @PagoUtilidadOperEdoGas	  DECIMAL(16,3),
    @PagoTotalEstadoPetroleo	  DECIMAL(16,3),
    @PagoTotalEstadoCondensado  DECIMAL(16,3),
    @PagoTotalEstadoGas		  DECIMAL(16,3),
    @PagoCtoElegibleContratistaPetroleo	DECIMAL(16,3),
    @PagoCtoElegibleContratistaCondensado   DECIMAL(16,3),
    @PagoCtoElegibleContratistaGas		DECIMAL(16,3),
    @PagoUtilidadOperContratistaPetroleo    DECIMAL(16,3),
    @PagoUtilidadOperContratistaCondensado  DECIMAL(16,3),
    @PagoUtilidadOperContratistaGas	DECIMAL(16,3),
    @CargoIngresosAdicionalesPetroleo	DECIMAL(16,3),
    @CargoIngresosAdicionalesCondensado	DECIMAL(16,3),
    @CargoIngresosAdicionalesGas		DECIMAL(16,3),
    @PagoTotalContratistaPetroleo		DECIMAL(16,3),
    @PagoTotalContratistaCondensado	DECIMAL(16,3),
    @PagoTotalContratistaGas			DECIMAL(16,3),
    @ComprobacionPetroleo	  DECIMAL(7,4),
    @ComprobacionCondensado	  DECIMAL(7,4),
    @ComprobacionGas		  DECIMAL(7,4),
    @PorcContraUtilidadOperContratista	DECIMAL(5,3),
    @PorcContraUtilidadOperEdo		DECIMAL(5,3)
...
SELECT
    @ProduccionPetroleo = VolumenPetroleoPuntoMedicion,
...
SELECT @CargoIngresosAdicionalesPetroleo    =   CASE WHEN ISNULL(@IngresoAdicional,0) > 0
...
SELECT @CargoIngresosAdicionalesCondensado    =   CASE WHEN ISNULL(@IngresoAdicional,0) > 0
...
SELECT @CargoIngresosAdicionalesGas    =   CASE WHEN ISNULL(@IngresoAdicional,0) > 0
...
--SELECT
--	  @ValorContracHidro 		 AS	[ValorContracHidro],
...
SELECT 1, 'Petróleo', @PagoTotalEstadoPetroleo+ @PagoTotalContratistaPetroleo,@PagoTotalEstadoPetroleo, @PagoTotalContratistaPetroleo
UNION
SELECT 2, 'Condensado',  @PagoTotalEstadoCondensado+ @PagoTotalContratistaCondensado, @PagoTotalEstadoCondensado, @PagoTotalContratistaCondensado
UNION
SELECT 3, 'Gas', @PagoTotalEstadoGas +@PagoTotalContratistaGas, @PagoTotalEstadoGas, @PagoTotalContratistaGas
--UNION
--SELECT 4, @PorcContraUtilidadOperEdo, @PorcContraUtilidadOperContratista
...
--	   @PagoTotalContratistaGas			   AS [PagoTotalContratistaGas],
    ...
    UPDATE #Resultado 
	   SET PorC   =  Contratista/ (COntratista + Estado), PorE   =  Estado/ (COntratista + Estado)
    WHERE
	   (COntratista <> 0 OR Estado <>0)

	   SELECT 
		--  Id,
		  Hidrocarburo,
		  	  FORMAT((ISNULL(PorE,0)),'p') AS	 PorcentajeDelEstado,
		  FORMAT((ISNULL(PorC,0)),'p')	  AS	 PorcentajeDelContratista,
		  FORMAT(ISNULL(Total,0), '###,###,###') 	  AS VolumenTotal,
		 FORMAT(ISNULL(Estado,0), '###,###,###') 	  AS VolumenDelEstado,
		 FORMAT( ISNULL(Contratista,0)	, '###,###,###')   AS	VolumenDelContratista
