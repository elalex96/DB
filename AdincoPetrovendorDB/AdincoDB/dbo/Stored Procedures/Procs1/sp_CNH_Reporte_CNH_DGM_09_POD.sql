...
-- 20210523 RIOA  Se agrega betweens y fechas para ver el reporte por medio de un rango de fechas dependiendo de la tabla: CO_InicioReportePODContrato
-- ==============================================================================================  
SET NOCOUNT ON  
-- ==============================================================================================  
...
(  
  IdContrato INT ,
  UltimoDiaMes DATE,
  ConCromatografia BIT
  PRIMARY KEY (IdContrato,UltimoDiaMes)
 --Temperatura FLOAT  
)  
...
DECLARE  
 ...
 IF((SELECT COUNT(1) FROM CO_InicioReportePODContrato WHERE IdContrato = @Contrato)>0)
 BEGIN
	SELECT @FechaInicio = FechaInicio  FROM CO_InicioReportePODContrato WHERE IdContrato = @Contrato AND Activo = 1;
 END
 ELSE
 BEGIN
	SELECT @FechaInicio = DATEADD(dd,-(DAY(@DiaReporte)-1),@DiaReporte);
 END
...
INSERT INTO #ContratosConciliada  --CONTRATO, MES
(  
 IdContrato  ,
 UltimoDiaMes
)  
...
JOIN 
	AP_CALENDARIO AC
    ON CONVERT(DATE,PDP.Fecha) = AC.IDFECHA
WHERE
    I.Activo = 1 AND
    PDP.Fecha BETWEEN @FechaInicio AND @DiaReporte
GROUP BY
    CO.IdContrato,
    AC.UltimoDiaMes
HAVING
    COUNT(DISTINCT PDP.Fecha) >= DAY(AC.UltimoDiaMes)
...
JOIN
    AP_Calendario AC
    ON AC.IDFECHA <= @DiaReporte -- FECHA REPORTE @DIAREPORTE
    AND AC.IDFECHA >= @FechaInicio -- FECHA INICIAL (LA QUE SE OBTIENE DE LA TABLA)
LEFT JOIN
    #ContratosConciliada    CC
    ON  C.IdContrato    =   CC.IdContrato
    AND AC.UltimoDiaMes = CC.UltimoDiaMEs
WHERE
...
UPDATE #ContratosDiaria
SET ConCromatografia = 0
WHERE ConCromatografia IS NULL


UPDATE #ContratosConciliada
SET ConCromatografia = 0
WHERE ConCromatografia IS NULL  
...
SELECT  
 ...
 DATEFROMPARTS(CR.Anio,CR.Mes,DAY(EOMONTH(DATEFROMPARTS(CR.Anio,CR.Mes,1))))   ,
 T.IdContrato
FROM  
 #Contratos T  
JOIN  
...
LEFT JOIN   
 ...
 AND DATEFROMPARTS(CR.Anio,CR.Mes,01) BETWEEN DATEFROMPARTS(YEAR(DATEADD(MONTH,-1,@FechaInicio)),MONTH(DATEADD(MONTH,-1,@FechaInicio)),01) AND  DATEFROMPARTS(YEAR(@DiaReporte),MONTH(@DiaReporte),DAY(EOMONTH(@DiaReporte)))  
LEFT JOIN  
...
UPDATE P
SET P.ConCromatografia = 1
FROM #ContratosDiaria P
JOIN #GradosAPI C
ON  P.UltimoDiaMes = C.UltimoDiaMes
AND P.IdContrato = C.IdContrato

UPDATE P
SET P.ConCromatografia = 1
FROM #ContratosConciliada P
JOIN #GradosAPI C
ON  P.UltimoDiaMes = C.UltimoDiaMes
AND P.IdContrato = C.IdContrato

/*
-- YA NO SE UTILIZA DEBIDO A QUE EL SIN CROMATOGRAFÍA YA ESTA EN LA TABLAS DE PRODUCCIONES   *************R
UPDATE #GradosAPI  
 SET SinCromatografia = 0  
WHERE  
 SinCromatografia IS NULL  
 */
--**************************************** YA NO SE REALIZA DEBIDO A QUE EN LA BUSQUEDA DE LA PRIMERA CROMATOGRAFÍA YA SE HIZO EL MENOS UN MES*****R
...
LEFT JOIN   
 dbo.CO_Cromatografia CR  
 ON PEC.IdContrato = CR.IdContrato  
	 AND DATEFROMPARTS(CR.Anio,CR.Mes,01) BETWEEN DATEFROMPARTS(YEAR(DATEADD(MONTH,-1,'20180301')),MONTH(DATEADD(MONTH,-1,'20180301')),01) AND  DATEFROMPARTS(YEAR('20210521'),MONTH('20210521'),DAY(EOMONTH('20210521')))  
--	AND DATEFROMPARTS(CR.Anio,CR.Mes,01) BETWEEN DATEFROMPARTS(YEAR(DATEADD(MONTH,-1,@FechaInicio)),MONTH(DATEADD(MONTH,-1,@FechaInicio)),01) AND  DATEFROMPARTS(YEAR(@DiaReporte),MONTH(@DiaReporte),DAY(EOMONTH(@DiaReporte)))  
LEFT JOIN  
 CO_CromatografiaValores CROMA  
 ON CR.IdCromatografia = CROMA.IdCromatografia  
...
JOIN  
 dbo.PR_ProdDiariaPozo_Previo PDP  
 ON P.Id = PDP.Pozo  
  AND 	T.UltimoDiaMes = DATEFROMPARTS(YEAR(PDP.FECHA),MONTH(PDP.FECHA),DAY(EOMONTH(PDP.FECHA))) 
JOIN  
 dbo.PR_Campo CPO  
 ON P.Campo = CPO.Id  
JOIN  
 #GradosAPI API  
 ON CPO.Id = API.IdCampo  
 AND	DATEFROMPARTS(YEAR(PDP.FECHA),MONTH(PDP.FECHA),DAY(EOMONTH(PDP.FECHA))) = API.UltimoDiaMes
 AND	T.ConCromatografia = 1
LEFT JOIN  
 #EventosCampo E  
...
GROUP BY  
 ...
 ISNULL(E.Eventos,'')  
  ...
  /*SIN CROMATOGRAFIA Y BUSCA EL MES ANTERIOR*/
 SELECT  
 CPO.Nombre AS [Normbre del Campo],  
 C.NumeroContrato AS [ID del Contrato o Asignación],  
 PDP.Fecha,  
...
JOIN  
 #GradosAPI API  
 ON CPO.Id = API.IdCampo  
 AND	DATEFROMPARTS(YEAR(PDP.FECHA),MONTH(PDP.FECHA),DAY(EOMONTH(PDP.FECHA))) = DATEADD(MONTH, -1, API.UltimoDiaMes)
 AND T.ConCromatografia = 0
LEFT JOIN  
 #EventosCampo E  
 ON PDP.Fecha = E.Fecha  
 AND CPO.Nombre = E.Campo  
