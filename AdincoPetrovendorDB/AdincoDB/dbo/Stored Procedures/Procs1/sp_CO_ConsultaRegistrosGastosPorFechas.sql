...
BEGIN
    ...
    SELECT DISTINCT
        PrimerDiaMes,
        UltimoDiaMes -- se buscan el primer día y el ultimo día del mes seleccionado, ya que se encontraron gastos con día del mes presentación mayor a 1
    ...
    WHERE SiguienteMesSeis > @DiaActual; --SE BORRAN EL MES QUE AUN NO SE PUEDE MOSTRAR HASTA DESPUES DEL DÍA 6 DEL SIGUIENTE MES

    IF ((SELECT COUNT(1) FROM #MesesAnio) > 0)
    BEGIN

        SELECT @FechaInicio = MIN(PrimerDiaMes),
