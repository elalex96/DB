...
-- Description:	Datos de linea de presupuesto concatenados para la reclasificacion de la aceptacion pedido detalle
-- =============================================
CREATE PROCEDURE [dbo].[CO_SP_ConsultaLineaPresupuestoMesv2_Reclasificacion_APD] --3
...
LEFT JOIN Adinco.dbo.CO_AnioContractual AC ON AC.IdAnioContractual = P.IdAnioContractual
		...
		SELECT 
			...
			'Linea de Presupuesto:' + CAST(IdLineaPresupuestoMes AS NVARCHAR) + ' (' + 'Mes Programado:' + Mes_Presupuestado + ' | ' + ' Actividad: ' + Actividad + ' | ' + ' SubActividad: ' + SubActividad + ' | ' + ' Tarea: ' + TareaPetrolera + ' | ' + 'Clave Tarea: ' + id_Tarea + ' | ' + 'SubTarea:' + SubactividadPetrolera + ' )' AS NombreLineaPresupuesto
		FROM #DATOSLINEAPRESUPUESTO
     END
