...
BEGIN TRY
    ...
    WITH MyCte
    AS (SELECT RFCProveedor
        FROM SC_Importacion
        GROUP BY RFCProveedor
       )
    INSERT INTO #Subcontratista
    (
        Repetidos,
        RfcProveedor
    )
    SELECT COUNT(RFCProveedor) Repetidos,
           RFCProveedor
    FROM MyCte
        INNER JOIN PV_Subcontratista
            ON UPPER(MyCte.RFCProveedor) = UPPER(PV_Subcontratista.RFC)
