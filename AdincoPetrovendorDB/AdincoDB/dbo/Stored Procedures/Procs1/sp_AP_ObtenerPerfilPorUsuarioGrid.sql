IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'sp_AP_ObtenerPerfilPorUsuarioGrid'
    )
    DROP PROCEDURE sp_AP_ObtenerPerfilPorUsuarioGrid;
GO
...
AS
    BEGIN
        ...
        WHERE
            ...
            AND PerfilID IN (
                                128, 416, 6
                            ); --Para saber si el usuario tiene el perfil administrador no es dependiente del contrato  
                ...
                FROM
                    ...
                    INNER JOIN
                        CO_Contrato              c (NOLOCK)
                            ON c.IdContrato = p.IdContrato
                    INNER JOIN
                        AP_Usuario               u2 (NOLOCK)
                            ON u2.UsuarioID = @UsuarioSesionId
                    INNER JOIN
                        AP_PerfilUsuario         pu2 (NOLOCK)
                            ON pu2.UsuarioID = u2.UsuarioID
                    INNER JOIN
                        AP_Perfil                p2 (NOLOCK)
                            ON p2.IdPerfil = pu2.PerfilID
