...
FROM
    #facturas   F
JOIN
    Adinco.dbo.CO_Registro AS RA (NOLOCK)
...
BEGIN
    ...
    JOIN Petrovendor.dbo.MM_Material MM (NOLOCK) ON pd.IdMaterial = MM.IdMaterial
    JOIN Petrovendor.dbo.AX_UnidadClasificacion auc (NOLOCK) ON  MM.IdUnidad = auc.IdUnidad AND auc.IdClasificacion = 1
    ...
    JOIN Adinco.dbo.CO_Registro AS RA (NOLOCK) ON F.IdFacturaAdinco = RA.IdFactura
    JOIN Adinco.dbo.CO_LineaPresupuestoMes lpm (NOLOCK) ON RA.IdPrograma = lpm.IdLineaPresupuestoMes
	JOIN
			Adinco.dbo.CO_Presupuesto PRE (NOLOCK) 
			ON lpm.IdPresupuesto = PRE.IdPresupuesto
	JOIN
			Adinco.dbo.CO_AnioContractual ANC (NOLOCK) 
