...
CREATE PROCEDURE [dbo].[P_AP_ValidarAccesoPorURLContratoUsuario]
    @UsuarioId INT,
    @ContratoId INT,
    @Url VARCHAR(500),
    @Permitir BIT OUT
...
BEGIN
	...
	FROM CO_Contrato where IdContrato = @ContratoId
  ...
  IF(@ContratistaId = 10013)--PERTENECE A CONTRATISTA DEA
	BEGIN
		SELECT  @Permitir ;
	END
	ELSE
