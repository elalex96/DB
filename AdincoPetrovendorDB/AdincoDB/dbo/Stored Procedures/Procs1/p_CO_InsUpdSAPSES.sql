...
as  
 ...
 if not exists (  
  ...
  SESNumber = @pSESNumber and  
  SESLine = @pSESLine and  
  PO_SAPNumer = @pPO_SAPNumer and  
  POLineNumber = @pPOLineNumber  
 )  
 begin  
    
   insert into [dbo].[CO_SAPSES](  
   IdContrato,SESNumber,SESLine,PO_SAPNumer,POLineNumber,  
   Quantity,UnitPrice,Currency,Importe,AccountAssignment,  
   CostObject,MaterialGroup,MaterialGroupDesc2,CreadoEl,CreadoPor,  
   UOM,  
   --  
   SESPostingDate,SESServiceStart,SESServiceEnd,Plant,  
   --  
   SESReferenceNumber  
   )  
   values(  
   @pIdContrato,@pSESNumber,@pSESLine,@pPO_SAPNumer,@pPOLineNumber,  
   @pQuantity,@pUnitPrice,@pCurrency,@pImporte,@pAccountAssignment,  
   @pCostObject,@pMaterialGroup,@pMaterialGroupDesc2,getdate(),@pCreadoPor,  
   @pUOM,  
   --  
   @pSESPostingDate,@pSESServiceStart,@pSESServiceEnd,@pPlant,  
   --  
   @pSESReferenceNumber  
   )  
 ...
 Begin  
  ...
  set Quantity = @pQuantity,  
   ...
   Plant = @pPlant,  
   SESReferenceNumber = case when isnull(SESReferenceNumber,'') = '' then  @pSESReferenceNumber  else SESReferenceNumber end
  where IdContrato = @pIdContrato and  
  SESNumber = @pSESNumber and  
  SESLine = @pSESLine and  
  PO_SAPNumer = @pPO_SAPNumer and  
