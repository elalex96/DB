...
AS
     ...
     (
         ...
         BEGIN
             RAISERROR(15600, -1, -1, 'No es posible volver a cerrar la semana');
             RETURN;
     END;
     IF NOT EXISTS
     (
         ...
         BEGIN
             RAISERROR(15600, -1, -1, 'No es posible cerrar la semana, Aún no hay captura de actividades por el Subcontratista');
             RETURN;
     END;
     BEGIN TRAN;
     IF NOT EXISTS
