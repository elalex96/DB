IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'p_OT_ConsultaSolicitudProgramaCaptura'
    )
    DROP PROCEDURE p_OT_ConsultaSolicitudProgramaCaptura;
GO
--************************************************************
...
CREATE PROCEDURE p_OT_ConsultaSolicitudProgramaCaptura
    @pIdOTSolicitud INT,
    @pSemana        VARCHAR(21),
    @pSoloVoBo      BIT = 0
AS
    BEGIN
                ...
                INSERT INTO #tmpSemana
                    (
                        Fecha
                            ...
                            SELECT
                                CONVERT(VARCHAR, OT_SolicitudProgramaCaptura.Fecha, 112)
                            FROM
                                OT_Solicitud (NOLOCK)
                                INNER JOIN
                                    [dbo].[OT_SolicitudMaterial] (NOLOCK)
                                        ON OT_Solicitud.idOTSolicitud = OT_SolicitudMaterial.IdOTSolicitud
										AND	  OT_Solicitud.IdOTSolicitud = @pIdOTSolicitud
                                INNER JOIN
                            ...
                            WHERE
                                OT_Solicitud.IdOTSolicitud = @pIdOTSolicitud
                            GROUP BY
                                CONVERT(VARCHAR, OT_SolicitudProgramaCaptura.Fecha, 112)
            END
        ...
        INSERT INTO #tmpDisponibles
            (
                ...
                Disponible
                    ...
                    SELECT
                        OT_SolicitudPrograma.IdOTSolicitudMaterial,
                    ...
                    FROM
                        ...
                        INNER JOIN
                            dbo.OT_SolicitudMaterial (NOLOCK)
								...
								AND	OT_SolicitudMaterial.IdOTSolicitud = @pIdOTSolicitud
                    WHERE
                        OT_SolicitudMaterial.IdOTSolicitud = @pIdOTSolicitud
                    GROUP BY
                        OT_SolicitudPrograma.IdOTSolicitudMaterial,
        ...
        INSERT INTO #tmpCaptura
            (
                ...
                Captura
                    ...
                    SELECT
                        #tmpDisponibles.IdOTSolicitudMaterial,
                        ISNULL(SUM(   CASE
        ...
        UPDATE
            #tmpDisponibles
        SET
            #tmpDisponibles.Disponible = Cantidad - #tmpCaptura.Captura
        FROM
            #tmpDisponibles
            INNER JOIN
                #tmpCaptura
        ...
        UPDATE
            #tmpResult
        SET
            #tmpResult.IdEstatus = ISNULL(OT_SolicitudPrograma.IdEstatus, 0)
        FROM
            #tmpResult
            INNER JOIN
                dbo.OT_SolicitudPrograma (NOLOCK)
        ...
        INSERT INTO #tmpDatos
            (
                ...
                DomingoCerrado
                    ...
                    SELECT
                        IdOTSolicitudMaterial = #tmpResult.IdOTSolicitudMaterial,
                        LunesCaptura          = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 2
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        MartesCaptura         = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 3
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        MiercolesCaptura      = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 4
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        JuevesCaptura         = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 5
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        ViernesCaptura        = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 6
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        SabadoCaptura         = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 7
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        DomingoCaptura        = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 1
                                                        THEN OT_SolicitudProgramaCaptura.Captura
                                                END,
                        -----------------------------  
                        LunesVoBoC            = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 2
                                                        THEN CAST(ISNULL(
                                                                            MAX(CAST(OT_SolicitudProgramaCaptura.VoBoContratista AS INT)),
                                                                            0
                                                                        ) AS BIT)
                                                    ELSE
                                                        0
                        ...
                        LunesCerrado          = CASE
                                                    WHEN DATEPART(WEEKDAY, OT_SolicitudProgramaCaptura.Fecha) = 2
                                                        THEN CAST(ISNULL(
                                                                            MAX(CAST(OT_SolicitudProgramaCaptura.Cerrado AS INT)),
                                                                            0
                                                                        ) AS BIT)
                                                    ELSE
                                                        0
                    ...
                    GROUP BY
                        #tmpResult.IdOTSolicitudMaterial,
        ...
        UPDATE
            #tmpResult
        SET
            #tmpResult.LunesCaptura = #tmpDatos.LunesCaptura,
        ...
        SET
            ...
            #tmpResult.ViernesVoBoC = #tmpVoBos.ViernesVoBoC,
            #tmpResult.SabadoVoBoC = #tmpVoBos.SabadoVoBoC,
            #tmpResult.DomingoVoBoC = #tmpVoBos.DomingoVoBoC,
            ---------------------------------  
            #tmpResult.LunesVoBoSC = #tmpVoBos.LunesVoBoSC,
            #tmpResult.MartesVoBoSC = #tmpVoBos.MartesVoBoSC,
            #tmpResult.MiercolesVoBoSC = #tmpVoBos.MiercolesVoBoSC,
        ...
        UPDATE
            #tmpResult
        SET
            #tmpResult.Disponible = ISNULL(#tmpDisponibles.Disponible, 0)
        FROM
            #tmpResult
            INNER JOIN
                #tmpDisponibles
        ...
        UPDATE
            #tmpResult
        SET
            #tmpResult.TieneArchivos = CASE
                                           WHEN #tmpArchivos.IdOTSolicitudMaterial > 0
                                               THEN CAST(1 AS BIT)
                                           ELSE
                                               CAST(0 AS BIT)
                                       END
