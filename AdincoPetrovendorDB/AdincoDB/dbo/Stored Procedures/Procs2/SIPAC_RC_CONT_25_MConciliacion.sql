...
(
    SELECT 1
    FROM dbo.sysobjects
    WHERE name = 'SIPAC_RC_CONT_25_MConciliacion'
)
    DROP PROCEDURE SIPAC_RC_CONT_25_MConciliacion;
GO

CREATE PROCEDURE [dbo].[SIPAC_RC_CONT_25_MConciliacion]
    @Contrato INT,
    @IdPresupuesto INT = 0,
...
BEGIN
    ...
    SELECT CASE
           ...
           CASE WHEN REPLACE(ISNULL(FI_PedimentoComprobante.IdDocFacturacionSIPAC, ''), '-', '_') = '' THEN '' ELSE CONCAT(REPLACE(FI_PedimentoComprobante.IdDocFacturacionSIPAC, '-', '_'), '.pdf') END AS [RC25_02],
