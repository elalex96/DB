...
--[SP_FI_FacturasPorContratista] 3,10002
				  ...
				  IdMoneda
                        ...
                        SELECT F.IdFactura, 
							...
							FROM dbo.FI_Factura AS F (NOLOCK)
                             ...
                             LEFT JOIN dbo.AWS_DocAwsDocAdinco WAD (NOLOCK) ON F.IdFactura = WAD.IdDocAdinco
...
--                        M.TipoMonedaCorto AS Moneda, 
					...
					FROM dbo.FI_Factura AS F (NOLOCK)
					 ...
					 JOIN dbo.PV_Subcontratista AS S (NOLOCK) ON F.IdSubcontratista = S.IdSubcontratista
                      --LEFT JOIN dbo.PV_TipoMoneda M (NOLOCK) ON M.IdMoneda = F.IdMoneda
...
--                        M.TipoMonedaCorto AS Moneda, 
                 ...
                 SELECT  
                   F.IdFactura, 
						...
						F.IdMoneda
				  FROM dbo.FI_Factura AS F (NOLOCK)
                      JOIN dbo.FI_FacturaContrato FC (NOLOCK) ON F.IdFactura = FC.IdFactura
                      JOIN dbo.CO_Contrato C (NOLOCK) ON FC.IdContrato = C.IdContrato
                      JOIN dbo.CO_Contratista CC (NOLOCK) ON C.IdContratista = CC.IdContratista
                                                    AND CC.IdContratista = @IdContratista
						JOIN dbo.PV_Subcontratista AS S (NOLOCK) ON F.IdSubcontratista = S.IdSubcontratista
                      --LEFT JOIN dbo.PV_TipoMoneda M (NOLOCK) ON M.IdMoneda = F.IdMoneda
