CREATE PROCEDURE [dbo].[SP_SE_A7_MPY] --10039,10109,10205,'20210101','20211201',10209,'Exploración'
    @IdContrato INT,
    @IdUsuario INT,
    @IdPresupuesto INT,
    @FInicio DATE,
	...
	*/
    ...
    WHERE (
      ...
      INSERT INTO #DATOS
		...
		WHERE  APD.ClasificacionCN = 6--Infraestructura Fisica Local y Regional
	 GROUP BY APD.DESCRIPCIONCORTA, FA.IdFactura;
    ...
    FROM dbo.CO_Registro R (NOLOCK)
        ...
        JOIN dbo.CO_TipoProgramaActividad TPA (NOLOCK)
            ON TPA.IdTipoProgramaActividad = PA.IdTipoProgramaActividad
    WHERE  R.IdGastoRubro = 6 --Infraestructura Fisica Local y Regional
          AND S.RFC NOT IN (
                               SELECT RFC FROM #RFC
                           )
