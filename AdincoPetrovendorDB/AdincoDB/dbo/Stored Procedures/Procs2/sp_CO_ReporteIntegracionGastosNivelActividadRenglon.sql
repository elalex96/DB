...
BEGIN
    ...
    FROM CO_LineaPresupuestoMes (NOLOCK)
        ...
        LEFT OUTER JOIN FI_PedimentoComprobante (NOLOCK)
            ON CO_Registro.IdPedimentoComprobante = FI_PedimentoComprobante.IdPedimentoComprobante
        LEFT OUTER JOIN CO_TipoCambioMensual CO_TipoCambioMensualPC (NOLOCK)
    ...
    UPDATE #Reporte
    SET DisplayRubro = NombreRubro
    FROM CO_Rubro
    WHERE CO_Rubro.IdRubro = NoRubro;
    --    
    UPDATE #Reporte
    SET DisplayServicio = NombreTipoServicio
    ...
    --    
    UPDATE #Reporte
    SET DesServicio = CO_Servicio.NombreServicio
    FROM CO_Servicio
    ...
    SELECT DisplayServicio,
           ...
           R.DisplayRubro
    FROM #Reporte R
        JOIN CO_TipoServicio TS (NOLOCK)
    ...
    FROM TempReporteIntegracionGastosNivelActividadRenglon GNAR (NOLOCK);
END;
