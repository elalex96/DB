IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'sp_OT_ConsultaSolicitudMateriales'
    )
    ...
    BEGIN
        ...
        select
            ...
            CantidadDisponible       = case
                                           when isnull(max(tmp2.CantidadSC), 0) - isnull(max(tmp2.CantidadOT), 0) < 0
                                               then 0
                                           else
            ...
            ModificadaPorContratista = cast(CASE
                                                WHEN SMB.IdOTSolicitudMaterial IS NOT NULL
                                                    THEN 1
                                                ELSE
                                                    0
                                            END AS bit),
            ...
            SM.Comentarios
        from
            #tmpCantidades2                          tmp2 (NOLOCK)
            JOIN
                dbo.SC_Materiales                    M (NOLOCK)
                    on tmp2.IdSCMaterial = M.IdSCMaterial
            inner join
                OT_Solicitud                         S (NOLOCK)
                    on M.IdSubContrato = S.IdSubContrato
                       AND S.IdOTSolicitud = @pIdOTSolicitud
            inner join
                sc_subcontrato                       SB (NOLOCK)
                    on S.idsubcontrato = SB.idsubcontrato
            left join
                Petrovendor.dbo.MM_Pedido            P (NOLOCK)
                    on SB.idPedido = P.IdPedido
            left join
                Petrovendor.dbo.PV_TipoMoneda        TM (NOLOCK)
            ...
            left JOIN
                Petrovendor.dbo.PV_MM_MaterialUnidad UM (NOLOCK)
                    ON M.IdUnidad = UM.IdUnidad
            LEFT JOIN
                dbo.OT_SolicitudMaterialBitacora     SMB (NOLOCK)
