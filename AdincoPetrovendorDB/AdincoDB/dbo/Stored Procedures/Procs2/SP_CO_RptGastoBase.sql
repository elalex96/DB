...
--[SP_CO_RptGastoBase] 10036,1,10183    
...
BEGIN  
	...
	--Se creo replica de tabla #MontoEjercidoPorLinea para usarlas solo en calculos de columnas Suma de Aceptacion de Servicios y  Suma de Aceptacion de Servicios pendientes
	CREATE TABLE #MontosDolares_Linea  
    (  
        id INT,  
        idLineaPresupuesto INT,  
        MontoTotalPedido DECIMAL,  
	...
	--Se creo replica de tabla #MontoEjercidoPorLinea para usarlas solo en calculos de columnas Suma de Aceptacion de Servicios y  Suma de Aceptacion de Servicios pendientes
	CREATE TABLE #MontoEjercidoPorLinea_2  
    (  
        Id INT,  
        IdLineaPresupuesto INT,  
        MontoTotalPedido DECIMAL,  
  ...
  LEFT JOIN dbo.CO_TipoCambioDiario TCDF  
	...
	/********************DISTINGUIR POR LINEA DE PRESUPUESTO Y GUARDAR EN TABLA #PedidoDetalle_Linea*****************/
	SELECT IdLineaPresupuestoMes = MAX(IdLineaPresupuestoMes),
		MontoDLS,
		IdPedido,
		IdPedidoDetalle,
		RazonSocial,
    ...
    SELECT P.IdLineaPresupuestoMes,  
           ...
           CASE  
                   WHEN ISNULL(PO.Cerrado, 0) = 1 THEN  
                       0  
                   ELSE  
                       SUM(ISNULL(P.MontoDLS, 0)) - ISNULL(SUM(PDA.MontoDLS),0)  
	...
	--Recalcular y almacenar en otra tabla usando como base #PedidoDetalle_Linea
	SELECT P.IdLineaPresupuestoMes,  
           ...
           P.IdSolicitudPedido,  
           P.Requisitor,  
           ISNULL(PDA.MontoDLS, 0) AS MontoAceptadoDLS,  
           CASE  
                   WHEN ISNULL(PO.Cerrado, 0) = 1 THEN  
                       0  
                   ELSE  
                       ISNULL(P.MontoDLS, 0) - ISNULL(PDA.MontoDLS,0)  
           END AS FaltantePorAceptar,  
           IdAceptacionFactura=0,--AF.IdAceptacionFactura,  
           CASE  
                   WHEN ISNULL(PO.Cerrado, 0) = 1 THEN  
                       ISNULL(P.MontoDLS, 0) - ISNULL(PDA.MontoDLS,0)  
                   ELSE  
               ...
               END AS MontoCerrado 	,
			   P.IdPedidoDetalle
    INTO #DatosPetrovendor_Linea  
    FROM #PedidoDetalle_Linea P  
        LEFT JOIN #PedidosDetalleAceptacionT PDA  
            ON PDA.IdPedidoDetalle = P.IdPedidoDetalle  
        LEFT JOIN Petrovendor.dbo.MM_Pedido PO  
            ON PO.IdPedido = P.IdPedido  
	...
	SELECT IdLineaPresupuestoMes,  
           MontoPedidoDetalleDLS = max(MontoPedidoDetalleDLS),  
           IdPedido,  
           NoPedido,  
           RazonSocial,  
           IdSolicitudPedido,  
           Requisitor,  
           MontoAceptadoDLS = SUM(MontoAceptadoDLS),  
           FaltantePorAceptar =  max(MontoPedidoDetalleDLS) -SUM(MontoAceptadoDLS) ,  
           IdAceptacionFactura,
           MontoCerrado,
		 IdPedidoDetalle
	INTO #DatosPetrovendor_Linea_2
	FROM #DatosPetrovendor_Linea
	GROUP BY IdLineaPresupuestoMes,IdPedidoDetalle,IdPedido,NoPedido,RazonSocial,IdSolicitudPedido,Requisitor,
	IdAceptacionFactura,MontoCerrado
	...
	INSERT INTO #MontosDolares_Linea  
    (  
        id,  
        idLineaPresupuesto,  
        MontoTotalPedido,  
	...
	INSERT INTO #MontoEjercidoPorLinea_2  
    (  
        Id,  
        IdLineaPresupuesto,  
        MontoTotalPedido,  
    ...
    FROM #Presupuestos PP  
		...
		LEFT JOIN #MontoEjercidoPorLinea_2 me2 ON me2.IdLineaPresupuesto = CO_LineaPresupuestoMes.IdLineaPresupuestoMes  
    GROUP BY CO_LineaPresupuestoMes.IdLineaPresupuestoMes,  
             ...
             me.IdPedido,  
             me.Proveedor  
    HAVING SUM(ISNULL(me.MontoTotalPedido, 0)) <> 0 /*OR  SUM(ISNULL(me2.MontoTotalPedido, 0))  <> 0*/ ; 

	SELECT Id = MAX(Id),
		LineaPresupuesto
	INTO #RptGastos_Lineas
	FROM #RptGastos
	GROUP BY LineaPresupuesto


	
	UPDATE #RptGastos
	SET MontoTotalPedido = (SELECT SUM(MontoTotalPedido) FROM #MontoEjercidoPorLinea_2 ST1 WHERE ST1.IdLineaPresupuesto = T1.LineaPresupuesto),
		MontoAceptado = (SELECT SUM(MontoAceptado) FROM #MontoEjercidoPorLinea_2 ST1 WHERE ST1.IdLineaPresupuesto = T1.LineaPresupuesto),
		MontoPendiente = (SELECT SUM(MontoPendiente) FROM #MontoEjercidoPorLinea_2 ST1 WHERE ST1.IdLineaPresupuesto = T1.LineaPresupuesto)
	FROM #RptGastos T1
	INNER JOIN #RptGastos_Lineas T2 ON T2.Id = T1.ID AND
									T2.LineaPresupuesto = T1.LineaPresupuesto	
