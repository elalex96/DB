CREATE PROCEDURE [dbo].[SP_CO_InformeRevisionGastosElegiblesNoProcedentes]
    @IdPresupuesto INT,
...
BEGIN
    SELECT
        -- R.IdRegistro,
        --ER.NombreEstado,
                TS.NombreTipoServicio AS 'Clasificación General',
                ...
                SF.RazonSocial AS 'Proveedor',
                -- R.IdFactura,
                F.Serie,
                F.Folio AS 'Factura',
                F.FechaTimbrado AS 'Fecha Factura',
                CASE
                ...
                R.MontoRegistro AS 'Monto Registro',
                F.TipoCambio AS 'T. C.',
                CASE
                     WHEN R.CvTipoDocFacturacion = 1 THEN
                         SUM(CASE
                                  WHEN ISNULL(R.MontoRegistro, 0) <> 0 THEN ISNULL(R.MontoRegistro, 0) / TCDF.TipoCambio
                                  ELSE 0 END)
                     WHEN R.CvTipoDocFacturacion IN ( 2, 3 ) THEN
                         SUM(CASE
                                  WHEN ISNULL(R.MontoRegistro, 0) <> 0 THEN ISNULL(R.MontoRegistro, 0) / TCDPC.TipoCambio
                                  ELSE 0 END) END AS 'Importe G.E. USD'
