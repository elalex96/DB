...
BEGIN
	...
	--		AND CE.IdContrato	=	@IdContrato
		...
		SELECT @ActConFechaReal = COUNT(1) FROM  EN_InstanciasActividades WHERE IdInstanciasProcesos = @IdInstanciaProceso AND 
			FechaRealActividad IS NOT NULL;
	...
	BEGIN	
		...
		CASE
			...
			THEN 
				'../../2/Entregables/subeHistorico.aspx?' + 'inst=' + LTRIM(@IdInstancia)
			WHEN
				R.Rol LIKE '%Entregables%Default%'
			THEN
				'../../2/Entregables/subeHistorico.aspx?' + 'inst=' + LTRIM(@IdInstancia)
			WHEN
				R.Rol LIKE '%Default%'
