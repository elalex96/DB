...
GROUP BY
    ...
    SELECT
		...
		CASE 
			...
			WHEN YEAR(M.Fecha) = 2022 AND DATEDIFF(YEAR, CO.FechaFirma, M.Fecha) < 5	THEN 1548.88
			WHEN DATEDIFF(YEAR, CO.FechaFirma, M.Fecha) > 5 THEN 3703.86
            ELSE 1396.09
        ...
        CASE 
			...
			WHEN DATEDIFF(YEAR, CO.FechaFirma, M.Fecha) > 5 THEN ROUND(3703.86 * AC.SuperficieKm2,2)
            ELSE 0
