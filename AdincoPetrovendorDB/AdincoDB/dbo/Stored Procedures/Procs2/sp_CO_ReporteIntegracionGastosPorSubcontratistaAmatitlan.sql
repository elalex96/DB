...
-- Create date: 23 de Noviembre del 2022    
-- Description: Issue-Adinco 2329 | Ajuste para Gastos en los cuales se haya registrado con un valor en Negativo ya sea por duplicidado por 
-- el registro de gasto a travez de una factura de Egreso, el simbolo de Negativo se visualice en la columna Importe Factura Original USD/MXP
-- =============================================
...
BEGIN
    ...
    SELECT UPPER(CONCAT(DATENAME(MONTH, R.MesPresentacion), ' ', YEAR(R.MesPresentacion))) AS FechaReporte,
           ...
           (CASE
                WHEN (F.TipoComprobante) LIKE '%egreso%'
                     ...
                     OR (ISNULL(RM.MontoGasto, R.MontoRegistro) < 0) THEN
                    CASE
            ...
            END
               ...
               ELSE
           (CASE
                WHEN ISNULL(RM.MontoGasto, R.MontoRegistro) < 0 THEN
                    CASE
                        WHEN ISNULL(R.Ajuste, '') = '' THEN
                            R.Comentarios
                        ELSE
           ...
           CASE
               WHEN ISNULL(MM.DescripcionLarga, '') = '' THEN
                   CASE
                       WHEN ISNULL(R.DescripcionPartidaServicio, '') = '' THEN
                           ISNULL(R.Comentarios, '') COLLATE Modern_Spanish_CI_AS
                       ELSE
           ...
           CASE
               ...
               WHEN R.CvTipoDocFacturacion IN ( 2, 3 ) THEN
                   ISNULL(RM.TipoCambio, TCMPC.TipoCambio)
               ELSE
                   0
           END AS TipoCambio,
           ...
           CASE
               WHEN ISNULL(RTRIM(PPS.IdPedido), '') = '' THEN
                   CASE
                       WHEN ISNULL(RTRIM(PG.IdPedido), '') = '' THEN
           ...
           END AS PUMXN,
           CAST(SUM(   CASE
                           WHEN R.MontoRegistro < 0
                                AND R.CvTipoDocFacturacion = 1 THEN
                               F.SubTotal * -1
                           WHEN R.MontoRegistro < 0
                                AND R.CvTipoDocFacturacion <> 1 THEN
                               PCD.PrecioUnitario * -1
                           WHEN R.CvTipoDocFacturacion = 1 THEN
                       (CASE
                            WHEN (F.TipoComprobante) LIKE '%egreso%'
    ...
    FROM dbo.CO_Registro AS R
        ...
        LEFT JOIN petrovendor..FI_Factura AS FPCO
            ON F.UUID = FPCO.UUID COLLATE Modern_Spanish_CI_AS
               AND FPCO.Activa = 1
        LEFT JOIN Petrovendor..CO_Registro AS RPCD
            ON FPCO.IdFactura = RPCD.IdFactura
               AND RPCD.IdAceptacionPedidoDetalle IS NULL
        LEFT JOIN Petrovendor..TA_Operacion AS TAO
