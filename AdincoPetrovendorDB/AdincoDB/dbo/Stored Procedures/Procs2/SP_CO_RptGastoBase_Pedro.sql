CREATE PROCEDURE [dbo].[SP_CO_RptGastoBase_Pedro] 
...
AS
     BEGIN
		 ...
		 CREATE TABLE #PedidoDetalleConAceptacion(IdPedidoDetalle INT, IdLineaPresupuestoMes INT, IdSolicitudPedido INT, IdSubcontratista INT, IdAceptacionPedido INT, Cantidad FLOAT)
   ...
   CASE
         ...
         (
             ...
             FROM Adinco.dbo.CO_Contrato C
                  JOIN Adinco.dbo.CO_Contratista CC ON CC.IdContratista = C.IdContratista
                  JOIN Petrovendor.dbo.S_Proveedor P ON CC.RFC = P.RFC COLLATE DATABASE_DEFAULT
         ...
         SELECT LPM.IdLineaPresupuestoMes, 
                SUM(CASE
                        WHEN PD.IdMoneda = 1
                        THEN Petrovendor.dbo.FN_PesosDolaresTipoCambio(PD.Subtotal, CAST(PD.CreadoEl AS DATE))
                        ELSE PD.Subtotal
                    END) AS MontoDLS, 
                P.IdPedido, 
                PD.IdPedidoDetalle, 
         ...
         FROM Petrovendor.dbo.MM_Pedido P
              JOIN Petrovendor.dbo.TA_Operacion AS O ON O.IdDocumento = P.IdSolicitudPedido
              JOIN Petrovendor.dbo.MM_PedidoDetalle PD ON PD.IdPedido = P.IdPedido --AND ISNULL(P.Activo,0) = 0
              JOIN Petrovendor.dbo.MM_PeticionOfertaDetalle POD ON POD.IdPeticionOfertaDetalle = PD.IdPeticionOfertaDetalle
              ...
              JOIN Petrovendor.dbo.MM_SolicitudPedido SP ON SP.IdSolicitudPedido = PO.IdSolicitudPedido
                                                            AND SP.IdSolicitudPedido = P.IdSolicitudPedido
              JOIN Petrovendor.dbo.MM_SolicitudPedidoDetalle SPD ON SPD.IdSolicitudPedido = SP.IdSolicitudPedido
                                                                    AND POD.IdSolicitudPedidoDetalle = SPD.IdSolicitudPedidoDetalle
              JOIN Petrovendor.dbo.MM_SolicitudPedidoDetalleLineaPresupuesto SPDLP ON SPDLP.IdSolicitudPedidoDetalle = SPD.IdSolicitudPedidoDetalle
              JOIN Adinco.dbo.CO_LineaPresupuestoMes LPM ON SPDLP.IdLineaPresupuesto = LPM.IdLineaPresupuestoMes
              JOIN Petrovendor.dbo.S_Proveedor S ON P.IdSubcontratista = S.IdProveedor AND P.IdSubcontratista IS NOT NULL
              JOIN Petrovendor.dbo.S_Usuario REQ ON SP.IdUsuarioSolicitante = REQ.IdUsuario
              JOIN #Presupuestos ON #Presupuestos.IdPresupuesto = LPM.IdPresupuesto
		 ...
		 SELECT PDt.IdPedidoDetalle, apdi.IdLineaPresupuesto,  pdt.IdSolicitudPedido, pdt.IdSubcontratista, ap.IdAceptacionPedido, apd.Cantidad
		 FROM #PedidoDetalle pdt
		JOIN Petrovendor.dbo.MM_AceptacionPedidoDetalle apd ON apd.IdPedidoDetalle = pdt.IdPedidoDetalle
		JOIN Petrovendor.dbo.MM_AceptacionPedidoDetalleInstalacion apdi ON apdi.IdAceptacionPedidoDetalle = apd.IdAceptacionPedidoDetalle
		JOIN Petrovendor.dbo.MM_AceptacionPedido ap ON ap.IdAceptacionPedido = apd.IdAceptacionPedido
		WHERE ISNULL(ap.IdEstatusEliminado, 0) = 0
		GROUP BY PDt.IdPedidoDetalle, apdi.IdLineaPresupuesto,  pdt.IdSolicitudPedido, pdt.IdSubcontratista, ap.IdAceptacionPedido, apd.Cantidad
		...
		DELETE #PedidoDetalle WHERE IdPedidoDetalle IN  (SELECT pdca.IdPedidoDetalle FROM #PedidoDetalleConAceptacion pdca)
		...
		INSERT INTO #PedidoDetalle
         ...
         SUM(CASE
          ...
          FROM #PedidoDetalleConAceptacion pdca
              JOIN Petrovendor.dbo.MM_PedidoDetalle PD ON PDCA.IdPedidoDetalle = PD.IdPedidoDetalle 
              JOIN Adinco.dbo.CO_LineaPresupuestoMes LPM ON pdca.IdLineaPresupuestoMes = LPM.IdLineaPresupuestoMes
              JOIN Petrovendor.dbo.S_Proveedor S ON pdca.IdSubcontratista = S.IdProveedor
			  JOIN Petrovendor.dbo.MM_SolicitudPedido sp ON sp.IdSolicitudPedido = pdca.IdSolicitudPedido
              JOIN Petrovendor.dbo.S_Usuario REQ ON sp.IdUsuarioSolicitante = REQ.IdUsuario
              JOIN #Presupuestos ON #Presupuestos.IdPresupuesto = LPM.IdPresupuesto
         GROUP BY LTRIM(RTRIM(S.RazonSocial)),
                  LTRIM(RTRIM(REQ.Nombre)),
         ...
         SELECT PD.IdPedido, 
                ...
                SUM(CASE
                        WHEN PD.IdMoneda = 1
                        THEN Petrovendor.dbo.FN_PesosDolaresTipoCambio(ISNULL(PD.PrecioUnitario * pdca.Cantidad, 0),
 CAST(PD.CreadoEl AS DATE))
                        ELSE PD.PrecioUnitario * pdca.Cantidad
                    END) AS MontoDLS,
					pdca.IdAceptacionPedido
         INTO #PedidosDetalleAceptacionT 
         FROM #PedidoDetalleConAceptacion pdca
        
      INNER JOIN Petrovendor.dbo.MM_PedidoDetalle PD ON PD.IdPedidoDetalle = pdca.IdPedidoDetalle
        group  BY PD.IdPedido,
         ...
         SELECT P.IdLineaPresupuestoMes, 
                SUM(ISNULL(p.MontoDLS, 0)) AS MontoPedidoDetalleDLS, 
                P.IdPedido, 
                ...
                SUM(CASE
                        WHEN ISNULL(PO.Cerrado, 0) = 1
                        THEN 0
                        ELSE ISNULL(P.MontoDLS, 0) - ISNULL(PDA.MontoDLS, 0)
                    END) AS FaltantePorAceptar, 
                AF.IdAceptacionFactura,
				SUM(CASE
                        WHEN ISNULL(PO.Cerrado, 0) = 1
                        THEN ISNULL(P.MontoDLS, 0) - ISNULL(PDA.MontoDLS, 0)
                        ELSE  0 
                    END) AS MontoCerrado
         INTO #DatosPetrovendor
         FROM #PedidoDetalle P
              LEFT JOIN #PedidosDetalleAceptacionT PDA ON PDA.IdPedidoDetalle = P.IdPedidoDetalle
              LEFT JOIN Petrovendor.dbo.MM_Pedido PO ON PO.IdPedido = p.IdPedido
              LEFT JOIN Petrovendor.dbo.MM_Pedidos PS ON PS.IdIdentificador = PO.IdPedido
                                                         AND PS.IdProveedorCliente = PO.IdProveedorCompras
              LEFT JOIN Petrovendor.dbo.MM_AceptacionFactura AF ON AF.IdAceptacionPedido = PDA.IdAceptacionPedido
              LEFT JOIN Petrovendor.dbo.TA_Operacion O ON O.IdDocumento = AF.IdAceptacionFactura AND PO.IdSubcontratista = O.IdProveedor
              LEFT JOIN Petrovendor.dbo.TA_Estatus T ON T.IdEstatus = O.IdEstatusOperacion
         ...
         (IdLineaPresupuesoMes, 
          Proveedores
                ...
                (
                    ...
                    WHERE B.IdLineaPresupuesto = A.IdLineaPresupuesto FOR XML PATH('')
                ), 1, 1, '') M --AND B.Id = A.Id
         ...
         SELECT #RptGastos.IdRegistro, 
                ...
                CASE
                    ...
                    THEN 0
                    ELSE((ISNULL(MontoTotalPedido, 0) + ISNULL([Registrado ADINCO], 0)) / ISNULL(PRESUP.PresupuestoUSD, 0))
