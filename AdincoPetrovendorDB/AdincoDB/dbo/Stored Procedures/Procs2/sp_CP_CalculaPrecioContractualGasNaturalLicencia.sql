CREATE PROCEDURE dbo.sp_CP_CalculaPrecioContractualGasNaturalLicencia
    @IdContrato INT = 0,
    @Mes        DATE,
	@IdUsuario	INT = 1
AS
...
(
	Hidrocarburo	INT,
	Metodo			VARCHAR(300),
	VolumenEntregado	FLOAT,
	VolumenComercializadoBaseReglasMercado	FLOAT,
	PorcentajeComercializado	FLOAT,
	PrecioContractualPetroleo	FLOAT
)

/*
IdTipoHidrocarburo	Hidrocarburo
10002				Metano
10003				Etano
10004				Propano
10005				Butano
...
--- SE EJECUTA EL PROCEDIMIENTO PARA CALCULAR EL PRECIO POR CADA COMPONENTE
INSERT INTO #Resultado
(
    Hidrocarburo,
    Metodo,
    VolumenEntregado,
    VolumenComercializadoBaseReglasMercado,
    PorcentajeComercializado,
    PrecioContractualPetroleo
)
EXEC sp_CP_CalculaPrecioContractualGasNatural_xComponente @IdContrato, @Mes, @IdUsuario, 10002	-- METANO
...
EXEC sp_CP_CalculaPrecioContractualGasNatural_xComponente @IdContrato, @Mes, @IdUsuario, 10003	-- ETANO
...
EXEC sp_CP_CalculaPrecioContractualGasNatural_xComponente @IdContrato, @Mes, @IdUsuario, 10004	-- PROPANO
...
EXEC sp_CP_CalculaPrecioContractualGasNatural_xComponente @IdContrato, @Mes, @IdUsuario, 10005	-- BUTANO
...
SELECT
    ...
    VolumenEntregado,
    VolumenComercializadoBaseReglasMercado,
    ROUND(PorcentajeComercializado,2,1) AS PorcentajeComercializado,
    PrecioContractualPetroleo
FROM
	#Resultado
ORDER BY
...
BEGIN
		UPDATE	#PrecioContractual
			SET	Regalia	=	(ROUND(PrecioContractual,2) / @Cn)*100
END
	...
	UPDATE	#PrecioContractual
		SET	Regalia	=	CASE WHEN PrecioContractual <= @Dn 
							...
							WHEN PrecioContractual > @Dn AND PrecioContractual < @En
								THEN (((ROUND(PrecioContractual,2) - @Dn) * 60.5)/ROUND(PrecioContractual,2) )
							WHEN PrecioContractual >= @En
								THEN (ROUND(PrecioContractual,2) / @Fn)*100
						END             
END
...
UPDATE    MC
	...
	Valor	=	PrecioContractual * VE.VolumenEntregado,
...
SELECT
    ...
    VE.VolumenEntregado,
	PrecioContractual * VE.VolumenEntregado AS ValorHidrocarburos,
	PC.Regalia
