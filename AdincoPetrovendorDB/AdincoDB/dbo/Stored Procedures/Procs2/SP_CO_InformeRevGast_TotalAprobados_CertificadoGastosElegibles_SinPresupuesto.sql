...
-- Descripción:		Se agrega el campo: Se retorna el toal de los gastos certificados de todos los presupuestos
--					de amatitlan
-- Url:				donde se ocupa /3/CIEP/CertificadoGastosElegiblesSinPresupuesto.aspx 
--====================================
CREATE PROCEDURE [dbo].[SP_CO_InformeRevGast_TotalAprobados_CertificadoGastosElegibles_SinPresupuesto]
    @MesPresentacion DATE
AS
BEGIN
    SET LANGUAGE SPANISH;
    DECLARE @IdContrato INT = 10007; -- Es el Id de Amatitlan esto ya que la pantalla solo la ocuparan ellos
    ...
    SELECT ISNULL(
                     SUM(   CASE
                                WHEN ISNULL(CO_RegistroMarkup.ImporteEstimadoParcial, 0) <> 0 THEN
                                    CAST(ISNULL(CO_RegistroMarkup.ImporteEstimadoParcial, 0)
                                         / (CASE
                                                WHEN CO_Registro.CvTipoDocFacturacion = 1 THEN
                                                    ISNULL(CO_RegistroMarkup.TipoCambio, TCDF.TipoCambio)
                                                WHEN CO_Registro.CvTipoDocFacturacion IN ( 2, 3 ) THEN
                                                    ISNULL(CO_RegistroMarkup.TipoCambio, TCDPC.TipoCambio)
                                                ELSE
                                                    0
                                            END
                                           ) AS DECIMAL(15, 2))
    ...
    FROM dbo.CO_LineaPresupuestoMes WITH (NOLOCK)
        ...
        JOIN CO_RegistroMarkup CO_RegistroMarkup (NOLOCK)
        ...
        LEFT JOIN dbo.PV_TipoMoneda AS TMF WITH (NOLOCK)
            ON TMF.IdMoneda = FI_Factura.IdMoneda
        LEFT JOIN dbo.CO_TipoCambioMensual AS TCDF WITH (NOLOCK)
            ON TCDF.IdMoneda = TMF.IdMoneda
               AND TCDF.Anio = YEAR(FI_Factura.Fecha)
               AND TCDF.IdMes = MONTH(FI_Factura.Fecha)
        LEFT JOIN dbo.PV_TipoMoneda AS TMPC WITH (NOLOCK)
            ON TMPC.IdMoneda = FI_PedimentoComprobante.IdMoneda
        LEFT JOIN dbo.CO_TipoCambioMensual AS TCDPC WITH (NOLOCK)
    ...
    WHERE CO_RegistroMarkup.MesEstadoPemex = @MesPresentacion
          AND CO_EstadoRegistro_V2.NombreEstado = 'Certificado GE Aprobado CACI'
