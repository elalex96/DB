CREATE PROCEDURE [dbo].[sp_SIPAC_PedimentoImportacion_RCCONT03M]
...
AS
     BEGIN
         ...
         SELECT con.IdSipac, 
                c.IdRegFiducidiario, 
                CONVERT(CHAR(10), (DATEFROMPARTS(YEAR(r.MesPresentacion), MONTH(r.MesPresentacion), 1)), 103) AS PeriodoReporte, 
                PC.IdDocFacturacionSIPAC AS IdentificadorDocumentoFacturacion, 
                PC.IdDocFacturacionSIPAC+'.pdf' AS NombreExtensionArchivo,
                CASE
                ...
                PC.NumeroPedimento, 
                PC.ClavePedimento, 
                CONVERT(CHAR(10), PC.FechaPago, 103) AS FechaPago, 
                PC.Regimen, 
                subi.RFC, 
                subi.RazonSocial, 
                PC.AduanaES AS 'Aduana E/S', 
                pcd.Cantidad, 
                pcd.DescripcionMercancia, 
                CAST(ROUND(pcd.PrecioUnitario, 2) AS DECIMAL(15, 2)) AS PrecioUnitario, 
                tm.TipoMonedaCorto AS MonedaFuncional, 
                CAST(ROUND(TCD.tipocambio, 4) AS DECIMAL(7, 4)) AS TipoCambioUSD, 
                CAST(ROUND(r.MontoRegistro, 2) AS DECIMAL(15, 2)) AS ValorComercial,
                CASE
         ...
         GROUP BY con.IdSipac, 
                  ...
                  CAST(ROUND(r.MontoRegistro, 2) AS DECIMAL(15, 2)),
                  CASE
                      WHEN ISNULL(r.MontoRegistro, 0) <> 0
                      THEN CAST(ROUND((ISNULL(r.MontoRegistro, 0) / TCD.TipoCambio), 2) AS DECIMAL(15, 2))
                      ELSE 0
         ...
         --EXEC sp_SIPAC_PedimentoImportacion_RCCONT03M 3,'2017-03-01',3
