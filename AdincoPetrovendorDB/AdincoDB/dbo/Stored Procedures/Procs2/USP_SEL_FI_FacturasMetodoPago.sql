IF EXISTS
    (
        ...
        FROM
            dbo.sysobjects
        WHERE
            name = 'USP_SEL_FI_FacturasMetodoPago'
    )
    DROP PROCEDURE USP_SEL_FI_FacturasMetodoPago
GO

CREATE PROCEDURE [dbo].[USP_SEL_FI_FacturasMetodoPago]
    @IdContrato INT,
    @IdUsuario INT,
	@FechaInicio DATE,
	@FechaFin DATE
    ...
    FROM dbo.FI_Factura (NOLOCK)
        JOIN dbo.PV_Subcontratista (NOLOCK)
            ON dbo.FI_Factura.IdSubcontratista = dbo.PV_Subcontratista.IdSubcontratista
				AND CONVERT(DATE, ISNULL(FI_Factura.FechaTimbrado, FI_Factura.Fecha))
