CREATE PROCEDURE [dbo].[SP_CO_SipacProduccionCalcular]
    @Idcontrato      INT,
...
SET NOCOUNT ON
--************************************************************************************************
EXEC SP_CO_SipacProduccionCalcular_V2 @Idcontrato, @fechaMesDiaAnio, @puntoEntrega, @Usuario
RETURN
...
SELECT
	@Idcontrato      AS IdContrato,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE C1 / @PM_Mezcla END AS C1,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE C2 /@PM_Mezcla END AS C2,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE C3 /  @PM_Mezcla  END AS C3,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE nC4 / @PM_Mezcla END AS nC4,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE iC4 / @PM_Mezcla END AS iC4,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE nC5 /@PM_Mezcla END AS nC5,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE IC5 / @PM_Mezcla END AS iC5,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE C6 / @PM_Mezcla  END AS C6,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE CO2 /  @PM_Mezcla END AS CO2,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE H2S / @PM_Mezcla END AS H2S,
	CASE WHEN @PM_Mezcla = 0 THEN 0 ELSE N2 / @PM_Mezcla  END AS N2
FROM
    #Calculo_Yi_PMi
...
SELECT
	...
	ISNULL( @ImportePetroleo / NULLIF(@PEP15_56Petroleo, 0), 0 ) AS Petroleo_Precio,	-- NUEVO PRECIO DE PETROLEO CONSIDERANDO LA CONVERSION DE TEMPERATURA
...
SELECT
    ...
    C3Venta,
    C4Venta,
    Condensado_Venta + Condensable_Venta AS [Condensado_Venta],
...
--IF @Idcontrato = 3 --IN (10001 , 10002, 10003 )
--BEGIN
--	SELECT @FechaLimite = '20181220'
--END 
IF (@Idcontrato = 10005 AND (@fechaMesDiaAnio = '2018-06-01' OR @fechaMesDiaAnio = '2018-07-01'))
BEGIN
SELECT @FechaLimite = '20190212'
END 
...
BEGIN
	...
	SELECT
		...
		CASE TH.IdTipoHidrocarburo
			...
			WHEN 10001	THEN SP.Condensado_Produccion + SP.Condensable_Produccion
		...
		CASE TH.IdTipoHidrocarburo
			...
			WHEN 10001	THEN SV.Condensado_Venta + SV.Condensable_Venta
	...
	SELECT
		SV.idContrato,
		@fechaMesDiaAnio,
		DATEADD(DAY,-1,DATEADD(MONTH,1,@fechaMesDiaAnio))		AS [FechaTransaccion],
		TH.IdTipoHidrocarburo,
		CASE TH.IdTipoHidrocarburo
			WHEN 10000	THEN SV.Petroleo_Venta
	...
	BEGIN
		...
		SELECT
			...
			SV.Condensable_Venta	AS	VolumenVendido,
			P.Condensado_Precios	AS	PrecioVentaUnitario,
			0,	--   CostoUnitarioComercializacion,
			P.Condensado_Precios	AS	PrecioPuntoMedicion,
			0,	--	IdFactura
			'NA',
			0,
			1,
