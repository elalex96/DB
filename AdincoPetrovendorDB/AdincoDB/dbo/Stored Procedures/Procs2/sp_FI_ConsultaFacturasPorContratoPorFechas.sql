...
CREATE PROCEDURE [dbo].[sp_FI_ConsultaFacturasPorContratoPorFechas]
...
/*
        ...
        CONVERT(DATE, F.CreadoEn) AS CreadoEn, 
             ...
             BEGIN
                ...
                inner JOIN dbo.PV_Subcontratista	S			--WITH(NOLOCK) 
				ON			F.IdSubcontratista		=			S.IdSubcontratista
				inner JOIN	dbo.CO_Contrato			C			--WITH(NOLOCK) 
				ON			F.IdContrato			=			C.IdContrato
                AND			(F.IdContrato			=			@IdContrato		OR	FC.IdContrato = @IdContrato	)
				and			F.Fecha					between		@FechaIni		and @FechaFin
                JOIN		dbo.CO_Contratista		CC			--WITH(NOLOCK) 
				ON			C.IdContratista			=			CC.IdContratista
				AND			CC.RFC					<>			F.Emisor
