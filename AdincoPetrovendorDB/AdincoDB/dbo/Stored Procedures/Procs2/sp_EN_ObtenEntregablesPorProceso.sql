...
-- =============================================
CREATE PROCEDURE [dbo].[sp_EN_ObtenEntregablesPorProceso]-- 3,10061,12788
    @idContrato INT,
    @idUsuario INT,
...
BEGIN
	...
	SELECT    DISTINCT ce.IdContratoEntregable, 
         ...
         WHERE      
			...
			AND      (   E.IdFrecuenciaEntregable IN ( 10018, 10019, 10016, 10011, 10010, 10008, 10004, 10003, 10000 )--EVENTO
			OR			 E.BitInterno         = 1)
			AND			 CE.Activo            = 1
			AND CE.DiasAlerta	>	0
			AND CE.DiasElaboracion	>	0
			AND CE.DiasRevision	>	0
			AND CE.DiasAprobacion	>	0
