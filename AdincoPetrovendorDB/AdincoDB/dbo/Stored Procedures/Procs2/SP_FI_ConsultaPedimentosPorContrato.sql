...
AS
	    ...
	    --
             ...
             FROM FI_PedimentoComprobante (NOLOCK)
                  INNER JOIN FI_PedimentoComprobanteDetalle (NOLOCK) ON FI_PedimentoComprobante.IdPedimentoComprobante = FI_PedimentoComprobanteDetalle.IdPedimentoComprobante
                  INNER JOIN dbo.PV_Subcontratista (NOLOCK) ON FI_PedimentoComprobante.IdSubcontratistaImportador = PV_Subcontratista.IdSubcontratista
                  INNER JOIN dbo.PV_Subcontratista PV_Subcontratista_Exportador (NOLOCK) ON FI_PedimentoComprobante.IdSubcontratistaExportador = PV_Subcontratista_Exportador.IdSubcontratista
                  INNER JOIN dbo.PV_TipoMoneda (NOLOCK) ON FI_PedimentoComprobante.IdMoneda = PV_TipoMoneda.IdMoneda 
			 ...
			 FROM #PedimentosPorContrato
			  JOIN dbo.FI_ClavesPedimento (NOLOCK) ON #PedimentosPorContrato.ClavePedimentoId = FI_ClavesPedimento.IdPedimento
			  ...
			  SELECT IdPedimento,
                    ...
                    PrecioUnitario,
                    Cantidad
					FROM #PedimentosPorContrato ORDER BY IdPedimento DESC
         END;
