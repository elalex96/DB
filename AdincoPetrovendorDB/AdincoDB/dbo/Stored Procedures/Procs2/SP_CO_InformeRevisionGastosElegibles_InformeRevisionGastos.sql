...
--====================================
CREATE PROCEDURE [dbo].[SP_CO_InformeRevisionGastosElegibles_InformeRevisionGastos]
    @IdPresupuesto   INT,
...
AS
    BEGIN
        ...
        SELECT
            ISNULL(CO_Presupuesto.Nombre, '')                                      AS 'Plan',
            ...
            CASE
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                    ...
                    THEN ISNULL(PV_Subcontratista_RefacturasF.RazonSocial, '')
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NULL
                    THEN ISNULL(PV_Subcontratista_Factura.RazonSocial, '')
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                                                             2, 3
                                                         )
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NOT NULL
                    THEN ISNULL(PV_Subcontratista_RefacturasPEPI.RazonSocial, '')
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                                                             2, 3
                                                         )
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NULL
                    THEN ISNULL(PV_Subcontratista_Extranjero.RazonSocial, '')
                ELSE
                    ''
            END                                                                    AS 'Proveedor',
            CASE
                ...
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NULL
                    THEN LTRIM(RTRIM(CONCAT(ISNULL(FI_Factura.Serie, ''), ' ', ISNULL(FI_Factura.Folio, ''))))
                ...
                WHEN CO_Registro.CvTipoDocFacturacion = 2
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NULL
                    THEN LTRIM(RTRIM(CONCAT('PI', ' ', ISNULL(FI_PedimentoComprobante.NumeroPedimento, ''))))
                ...
                WHEN CO_Registro.CvTipoDocFacturacion = 3
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NULL
                    THEN LTRIM(RTRIM(CONCAT('PE', ' ', ISNULL(FI_PedimentoComprobante.FolioComprobante, ''))))
                ELSE
                    ''
            END                                                                    AS 'Factura',
            CASE
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NOT NULL
                     AND FI_Factura_RefacturasF.FechaTimbrado IS NOT NULL
                    THEN CONCAT(
                ...
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NULL
                     AND FI_Factura.FechaTimbrado IS NOT NULL
                    THEN CONCAT(
                ...
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                     ...
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NOT NULL
                     AND FI_Factura_RefacturasPEPI.FechaTimbrado IS NOT NULL
                    THEN CONCAT(
                                   RIGHT('00'
                                         + CAST(DATEPART(DD, FI_Factura_RefacturasPEPI.FechaTimbrado) AS VARCHAR(2)), 2),
                                   '/',
                                   RIGHT('00'
                                         + CAST(DATEPART(MM, FI_Factura_RefacturasPEPI.FechaTimbrado) AS VARCHAR(2)), 2),
                ...
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                     ...
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NULL
                     AND FI_PedimentoComprobante.FechaPago IS NOT NULL
                    THEN CONCAT(
            ...
            CASE
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NOT NULL
                    THEN PV_TipoMoneda_RefacturasF.TipoMonedaCorto
                WHEN CO_Registro.CvTipoDocFacturacion = 1
                     AND FI_Factura_RefacturasF.IdFactura IS NULL
                    THEN PV_TipoMoneda_Factura.TipoMonedaCorto
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                                                             2, 3
                                                         )
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NOT NULL
                    THEN PV_TipoMoneda_RefacturasPEPI.TipoMonedaCorto
                WHEN CO_Registro.CvTipoDocFacturacion IN (
                                                             2, 3
                                                         )
                     AND FI_Factura_RefacturasPEPI.IdFactura IS NULL
                    THEN PV_TipoMoneda.TipoMonedaCorto
                ELSE
                    ''
            END                                                                    AS Moneda,
            ...
            ISNULL(
                      CAST(CASE
                               WHEN ISNULL(CO_RegistroMarkup.ImporteEstimadoParcial, 0) <> 0
                                   THEN CAST(ISNULL(CO_RegistroMarkup.ImporteEstimadoParcial, 0)
                                             / (CASE
                                                    WHEN CO_Registro.CvTipoDocFacturacion = 1
