if exists (select * from sys.procedures where name = 'SP_Ins_WDEA_Bitacora_AdincoSAP')
begin
	drop proc SP_Ins_WDEA_Bitacora_AdincoSAP
end
...
begin
		...
		create table #tmpMaterialesEncontrados
		(
			Id					int,
			IdMaterial			int,
			DescripcionCorta	varchar(max)
		...
		/*Justificacion*/
			...
			on			t1.Short_Text	=	t2.DescripcionCorta
			where		t2.DescripcionCorta	is null

			--select '#tmpMateriales', * from #tmpMateriales
