CREATE PROCEDURE [dbo].[sp_PR_CalcularDiferida] 
...
select @inicio =DATEADD (DAY,1,  @Fecha)
select @fin = DATEADD (DAY,1,  @Fecha)
...
FROM         PR_ParoDetalle INNER JOIN
        PR_ProdDiariaPozo 
	   ON PR_ParoDetalle.ProdDiaria = PR_ProdDiariaPozo.ProdDiaria INNER JOIN
        PR_ControlPozo 
	   ON PR_ProdDiariaPozo.ControlUtilizado = PR_ControlPozo.Id INNER JOIN
        PR_Paro 
	   ON PR_ParoDetalle.IdParo = PR_Paro.Id AND PR_ProdDiariaPozo.Pozo = PR_Paro.Pozo AND PR_ControlPozo.Pozo = PR_Paro.Pozo
END
