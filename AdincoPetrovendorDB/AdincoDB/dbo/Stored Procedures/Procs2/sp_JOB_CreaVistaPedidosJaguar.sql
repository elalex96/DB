...
JOIN  
 ...
 AND P.IdProveedorCompras = O.IdProveedor --IN (606, 690, 1835)  
...
WHERE  
 P.IdProveedorCompras IN (606, 690, 1835, 863)-- JAGUAR Y CARDENAS-MORA  
GROUP BY  
 C.IdContrato,  
 C.NumeroContrato,  
 P.IdPedido,  
...
JOIN  
 ...
 AND PG.IdProveedorCliente IN (606, 690, 1835, 863)  
JOIN  
...
SELECT  
 V.IdConsecutivo, V.IdPedido, V.SolicitudPedido, SPDL.IdLineaPresupuesto, APR.Nombre  
FROM #Vista V  
JOIN  
...
JOIN Adinco.dbo.CO_LineaPresupuestoMes ALP (NOLOCK)
 ON SPDL.IdLineaPresupuesto = ALP.IdLineaPresupuestoMes
JOIN Adinco.dbo.CO_Presupuesto APR (NOLOCK)
 ON ALP.IdPresupuesto = APR.IdPresupuesto
GROUP BY  
...
FROM  
 #VistaFinal VF  
JOIN  
 Adinco.dbo.FI_FacturaAdincoPetrovendor FAP  
...
WHERE  
 EstatusCN IS NULL  
 AND  
 IdAceptacionPedido IS NOT NULL  
...
UPDATE VF  
 SET   
  EstatusPago = 'Pagado SIN COMPLEMENTO'  
FROM   
 #VistaFinal    VF  
JOIN  
 ...
 WHERE VF.metodopago = 'PPD'  AND vf.estatuspago = 'Pagado'  
...
SELECT  
 ...
 IdPedido,  
 SolicitudPedido,  
 STUFF(( SELECT  ', '+ A.Presupuesto FROM #ClasificacionPresupuesto A  
   WHERE B.IdConsecutivo = A.IdConsecutivo AND B.IdPedido = A.IdPedido AND B.SolicitudPedido = A.SolicitudPedido FOR XML PATH('')),1 ,1, '')  Members  
FROM  
 #ClasificacionPresupuesto B  
GROUP BY  
...
UPDATE V  
 SET V.Presupuesto = I.Descripcion  
FROM  
 #VistaFinal V  
JOIN  
 #Clasificacion I  
...
SELECT     IdConsecutivo,     IdFactura,      SUBSTRING(STUFF(( SELECT '|'+ RTRIM(LTRIM(Concepto)) FROM #Conceptos A             WHERE B.IdConsecutivo = A.IdConsecutivo AND B.IdFactura = A.IdFactura FOR XML PATH('')),1 ,1, ''),1,8000) FROM     #Conceptos
 B GROUP BY     IdConsecutivo,     IdFactura  
...
(idFactura,   
 ...
 idOperacion  
       ...
       FROM fi_factura ff  
            ...
            JOIN dbo.S_Proveedor sp ON sp.RFC COLLATE SQL_Latin1_General_CP1_CI_AS = ff2.Emisor  
       ...
       WHERE TAO.IdEstatusOperacion = 2  
             AND ff2.Receptor IN('JEP1709042B1', 'PEP170906DI5', 'JSE1601292U8')  
...
FROM VISTA_ComprasDirectas cd  
     JOIN #FechasAprobacion FA ON FA.idOperacion = cd.idoperacion  
...
FROM VISTA_ComprasDirectas cd  
     JOIN #FacturaConceptos FC ON cd.idFactura = FC.IdFactura  
/*************** VISTA FACTURAS COMPRA DIRECTA **************************************/  
...
FROM #VistaFinal V  
JOIN  
 MM_AceptacionPedidoDetalle APD  
 ON V.IdAceptacionPedido = APD.IdAceptacionPedido  
JOIN  
 MM_AceptacionPedidoDetalleInstalacion APDI  
