USE [Petrovendor]
GO
/****** Object:  StoredProcedure [dbo].[SP_DEA_GetDocumento_SolicitudAceptacionServicio]    Script Date: 30/03/2022 12:09:49 a. m. ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @DOCUMENTO_TIPO_PROFORMA INT = (SELECT TOP 1 IdTipoDocumento FROM S_TipoDocumento WHERE NombreTipoDocumento = 'PROFORMA');
	DECLARE @DOCUMENTO_TIPO_FIELD_TICKET INT = (SELECT TOP 1 IdTipoDocumento FROM S_TipoDocumento WHERE NombreTipoDocumento = 'FIELD TICKET');
	...
	SELECT
		'PROFORMA_ ' + SUBSTRING(REPLACE(DS3.NombreDocumento,DS3.Extension,''),0,20) + DS3.Extension,
		DS3.Extension,
		DS3.Mime,
		DS3.Carpeta,
		DS3.Identificador,
	...
	JOIN S_Documento_S3 AS DS3
		...
		AND DS3.IdTipoDocumento = @DOCUMENTO_TIPO_PROFORMA
		AND DS3.Activo = 1;

	--INSERTAR FIELD TICKET
	INSERT INTO #DOCUMENTOS_SOLICITUD_ACEPTACION
	SELECT
		'FIELDTICKET_' + SUBSTRING(REPLACE(DS3.NombreDocumento,DS3.Extension,''),0,20) + DS3.Extension,
		DS3.Extension,
		DS3.Mime,
		DS3.Carpeta,
		DS3.Identificador,
	...
	JOIN S_Documento_S3 AS DS3
		ON DS3.IdDocumentoTabla = SAP.IdSolicitudAceptacionPedido
		AND DS3.IdTipoDocumento = @DOCUMENTO_TIPO_FIELD_TICKET
		AND DS3.Activo = 1;
