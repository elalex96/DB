CREATE PROCEDURE [dbo].[sp_COM_CalculaComercializacionesGas]
    @IdContrato INT,
    @MesReporte    DATE,
	@Usuario	INT,
...
(
    ...
    IdFactura                     INT,
    NumeroFolioPedimento          VARCHAR (100),
    EPT                           INT,
    OperacionBajoReglasMercado    INT,
    ...
    FechaFactura                  VARCHAR (100),
    --UUID                          VARCHAR (100),
    IdDocFacturacionSIPAC         VARCHAR (100),
...
--CREATE TABLE #OperacionComercializacion
--(
--	FechaTransaccion	DATETIME,
...
-- ------------------------------------------
DECLARE
    @FactorConversion     DECIMAL (8, 6),
	@pcC1						DECIMAL (8, 2),
	@pcC2						DECIMAL (8, 2),
	@pcC3						DECIMAL (8, 2),
	@pcC4						DECIMAL (8, 2),
	@pcIC4						DECIMAL (8, 2),
	@PC_iPentano				DECIMAL (8, 2),
	@PC_nPentano				DECIMAL (8, 2),
	@PC_nHexano					DECIMAL (8, 2),
	@PC_nHeptano				DECIMAL (8, 2),
	@PC_nOctano					DECIMAL (8, 2),
	@Conversion60FaR			DECIMAL (8, 2),
	@Conversion20CaR			DECIMAL (8, 2),
	@NumError					INT,
...
SELECT
	@VolC1 = CASE	WHEN @EsPC = 1 AND @EsConsorcio = 1  
						THEN C1 * @PorcPemex
					ELSE	C1
			END,
	@VolC2 = CASE	WHEN @EsPC = 1 AND @EsConsorcio = 1  
						THEN C2 * @PorcPemex
					ELSE	C2
			END,
	@VolC3 = CASE	WHEN @EsPC = 1 AND @EsConsorcio = 1  
						THEN C3 * @PorcPemex
					ELSE	C3
			END,
	@VolC4 = CASE	WHEN @EsPC = 1 AND @EsConsorcio = 1  
						THEN C4 * @PorcPemex
					ELSE	C4
			END,
	@VolC5 = CASE	WHEN @EsPC = 1 AND @EsConsorcio = 1  
						THEN C5 * @PorcPemex
					ELSE	C5
			END
FROM
...
JOIN
	...
	AND C.IdPtoExpedicionRecepcion = A.IdPtoExpedicionRecepcion
WHERE
	(A.C1Ajuste < 0 OR A.C2Ajuste < 0 OR A.C3Ajuste < 0 OR A.C4Ajuste < 0)
...
--	SELECT @VolTotalC1 AS '@VolTotalC1', @VolTotalC2 AS '@VolTotalC2', @VolTotalC3 AS '@VolTotalC3', @VolTotalC4 AS '@VolTotalC4'
--END

/*
*** SE COMENTA TODA ESTA PARTE PORQUE NO SE USA LA TABLA #OperacionComercializacion
INSERT INTO #OperacionComercializacion
(
	FechaTransaccion,
	PrecioVentaUnitario,
	PrecioPuntoMedicion,
...
SELECT
    ...
    0			 AS PrecioPuntoMedicion,		-- SE CALCULA MAS ABAJO PARA PODER HACER EL JOIN CON LA TABLA DE COSTOUNITARIOCOMERCIALIZACION
    F.IdFactura,
	MMBTUC1FacturadosBloqueEntero,
    MMBTUC2FacturadosBloqueEntero,
    MMBTUC3FacturadosBloqueEntero,
...
IF @NumError <> 0
BEGIN
	SELECT @MensajeError = 'Error al insertar en #OperacionComercializacion'+
	'En el Stored Procedure: dbo.sp_COM_CalculaComercializacionesGas '+
	'Núm. Error en SQL Server:'+CHAR(9)+LTRIM(STR(@NumError, 10, 0));
	GOTO ERROR
END	-- IF @NumError <> 0
...
BEGIN
	SELECT
		...
		@MesReporte                                                                          AS MesReporte, --MesReporte
		COM.FechaTransaccion,
		Tipo.IdTipoHidrocarburo                                                              AS IdTipoHidrocarburo, --     IdTipoHidrocarburo     10002: Metano      10003: Etano       10004: Propano  10005: Butano
		CASE
			WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	COM.MMBTUC1FacturadosBloqueEntero
			WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	COM.MMBTUC2FacturadosBloqueEntero
			WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	COM.MMBTUC3FacturadosBloqueEntero
			WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	COM.MMBTUC4FacturadosBloqueEntero
		END																					AS	VolumenVendido,
		COM.PrecioVentaUnitario,
		Tipo.CostoUnitarioComercializacion,
		COM.PrecioVentaUnitario - Tipo.CostoUnitarioComercializacion						AS [PrecioPuntoMedicion],--COM.PrecioPuntoMedicion,
		COM.IdFactura,
		...
		@Usuario                                                                             AS CreadoPor, --CreadoPor
		GETDATE()                                                                            AS CreadoEl, --CreadoEl
		@Usuario                                                                             AS ModificadoPor, --ModificadoPor
		GETDATE()				AS ModificadoEl, --ModificadoEl
		1                     AS Activo, ---Activo
		0						AS PVUAnterior,
		0                       AS PPMAnterior
	FROM
		#OperacionComercializacion	COM
	CROSS JOIN
...
*/
	...
	SELECT
		...
		1                                                                                    AS OperacionBajoReglasMercado, --OperacionBajoReglasMercado
		2                                         AS ClasificacionDocumentoSoporte, --ClasificacionDocumentoSoporte
		--1                                                                                    AS CreadoPor, --CreadoPor
		--GETDATE()      AS CreadoEl, --CreadoEl
		--1                                                                                    AS ModificadoPor, --ModificadoPor
		--GETDATE()                                                                            AS ModificadoEl, --ModificadoEl
		1                                                               AS Activo, ---Activo
		0                                                                                    AS PVUAnterior,
		0                                                                                    AS PPMAnterior,
		...
		--F.Subtotal / TCD.TipoCambio          AS Ingreso,
		--PTI.Total / TCD.TipoCambio          AS Ingreso,
		CONVERT( FLOAT, CRO.C1 )                                                             AS C1,
		CONVERT( FLOAT, CRO.C2 )                                                             AS C2,
		CONVERT( FLOAT, CRO.C3 )                                                             AS C3,
		CONVERT( FLOAT, CRO.IC4 )                                                            AS IC4,
		CONVERT( FLOAT, CRO.NC4 )  AS NC4,
		CONVERT( FLOAT, CRO.IC5 )                                                            AS IC5,
		CONVERT( FLOAT, CRO.NC5 )                                                            AS NC5,
		CONVERT( FLOAT, CRO.C6 )                                                             AS C6,
		...
		((CONVERT( FLOAT, CRO.C1 ) * @pcC1 ) + (CONVERT( FLOAT, CRO.C2 ) * @pcC2)
		+ (CONVERT( FLOAT, CRO.C3 ) * @pcC3) + (CONVERT( FLOAT, CRO.NC4 ) * @pcC4)
		+ (CONVERT( FLOAT, CRO.IC4 ) * @pcIC4)
		+ (CONVERT( FLOAT, CRO.IC5 ) + CONVERT( FLOAT, CRO.NC5 ) + CONVERT( FLOAT, CRO.C6 ))
	...
	JOIN
		...
		AND F.IdMoneda     = TCD.IdMoneda
	JOIN
		fi_cfdicromatografia       CRO
		ON CRO.IdFacturaConcepto    = FC.IdFacturaConcepto
	WHERE
		( R8.factura LIKE '92%'  OR   R8.Factura LIKE '93%'  )
		AND (R8.Denominación LIKE '%gas %')
		AND R8.CantidadFacturada > 0
...
END	-- IF @NumError <> 0
/*
C1		C2		C3		C4		IC4		C5+			CO2			H2S		N2
0.0116	0.0238	0.0347	0.047	0.0441	0.072735	0.0195	0.0239	0.00442
*/
...
SELECT
	...
	(EnerC1 * (pcC1 / Total * Ingreso)) + (EnerC2 * (pcC2 / Total * Ingreso )) +
		(EnerC3 * (pcC3 / Total * Ingreso)) + (EnerC4 * (((((pcC4 / Total) * Ingreso) * NC4) + (pcIC4 / Total * Ingreso * IC4)) / (NC4 + IC4)))	AS [IngresoC1C4],
	Ingreso - ((EnerC1 * (pcC1 / Total * Ingreso)) + (EnerC2 * (pcC2 / Total * Ingreso )) +
		(EnerC3 * (pcC3 / Total * Ingreso)) + (EnerC4 * (((((pcC4 / Total) * Ingreso) * NC4) + (pcIC4 / Total * Ingreso * IC4)) / (NC4 + IC4)))) AS [IngresoC5],
	...
	(IC5+NC5+C6) AS [Q5],
	((IC5+NC5+C6)/100*1000) * (1/25.32) * (14.696 / 14.696) AS [LCIdeal],
...
--	(((IC5+NC5+C6)/100*1000) * (1/25.32) * (14.696 / 14.696)) / 
...
--	MMPC60F * 1000 * ((((IC5+NC5+C6)/100*1000) * (1/25.32) * (14.696 / 14.696)) / 
...
--	(Ingreso - ((EnerC1 * (pcC1 / Total * Ingreso)) + (EnerC2 * (pcC2 / Total * Ingreso )) +
--		(EnerC3 * (pcC3 / Total * Ingreso)) + (EnerC4 *(((((pcC4 / Total) * Ingreso) * NC4) + (pcIC4/Total * Ingreso * IC4)) / (NC4 + IC4))))) / 
--		(MMPC60F * 1000 * ((((IC5+NC5+C6)/100*1000) * (1/25.32) * (14.696 / 14.696)) / 
--		(1- POWER(((C1/100)*0.0116) + ((C2/100)*0.0238) + ((C3/100)*0.0347) + ((NC4/100)*0.047) + ((IC4/100)*0.0441) + (((IC5+NC5+C6)/100)*0.072735) + ((CO2/100)*0.0195) + ((H2S/100)*0.0239) + ((N2/100)*0.00442) ,2)* 14.696)) / 42.00) AS [PrecioEquivCondensado]
FROM
	#PreciosGas2
CROSS JOIN #SumEner
...
BEGIN
	...
	CROSS JOIN #SumEner
	--SELECT 'ingreso c5+'
	--SELECT  * FROM #NVOCALCULO
	SELECT	* FROM #PrecioObjetivo
	SELECT * FROM #CalculoPreciosObjetivo
END
...
SELECT @SumC1 = 0, @SumC2 = 0, @SumC3 = 0, @SumC4 = 0
SELECT
...
SELECT
	((((@SumC1 - SUM(((MMBTUC1FacturadosBloqueEntero * ((pcC1/Total * Ingreso)))/@VolTotalC1))) * 100)/SUM(((MMBTUC1FacturadosBloqueEntero * ((pcC1/Total * Ingreso)))/@VolTotalC1))/100)),
	((((@SumC2 - SUM(((MMBTUC2FacturadosBloqueEntero * ((pcC2/Total * Ingreso)))/@VolTotalC2))) * 100)/SUM(((MMBTUC2FacturadosBloqueEntero * ((pcC2/Total * Ingreso)))/@VolTotalC2))/100)),
	((((@SumC3 - SUM(((MMBTUC3FacturadosBloqueEntero * ((pcC3/Total * Ingreso)))/@VolTotalC3))) * 100)/SUM(((MMBTUC3FacturadosBloqueEntero * ((pcC3/Total * Ingreso)))/@VolTotalC3))/100)),
	((((@SumC4 - SUM(((MMBTUC4FacturadosBloqueEntero * ((pcC4/Total * Ingreso)))/@VolTotalC4))) * 100)/SUM(((MMBTUC4FacturadosBloqueEntero * ((pcC4/Total * Ingreso)))/@VolTotalC4))/100))
FROM
	 #PreciosGas2	P
--CROSS JOIN
--	#VolTotal	VT
...
UPDATE	P
	SET DescuentoPrecioBalanceoC1 = FactorVolumenPrecioC1 * ((pcC1/Total*Ingreso)/FactorVolumenC1),
		DescuentoPrecioBalanceoC2 = FactorVolumenPrecioC2 * ((pcC2/Total*Ingreso)/FactorVolumenC2), 
		DescuentoPrecioBalanceoC3 = FactorVolumenPrecioC3 * ((pcC3/Total*Ingreso)/FactorVolumenC3),
		DescuentoPrecioBalanceoC4 = FactorVolumenPrecioC4 * ((pcC4/Total*Ingreso)/FactorVolumenC4)
FROM
		#PreciosGas2	P
	--CROSS JOIN
	--	#CostoUnitarioComercializacion		Tipo
...
--		CASE
--			WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	(pcC1/Total*Ingreso)
--			WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	(pcC2/Total*Ingreso)
--			WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	(pcC3/Total*Ingreso)
--			WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	(pcC4/Total*Ingreso)
--		END							AS	PrecioVentaUnitarioACTUAL,
--		CASE
--			WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	(MMBTUC1FacturadosBloqueEntero * (pcC1/Total*Ingreso) )/@VolTotalC1
--			WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	(MMBTUC2FacturadosBloqueEntero * (pcC2/Total*Ingreso) )/@VolTotalC2			 
--			WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	(MMBTUC3FacturadosBloqueEntero * (pcC3/Total*Ingreso) )/@VolTotalC3
--			WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	(MMBTUC4FacturadosBloqueEntero * (pcC4/Total*Ingreso) )/@VolTotalC4
--		END							AS PONDERACIONACTUAL,
--		CASE
...
--		END							AS VOLUMEN,
--		CASE
--			WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	CONVERT(FLOAT,MMBTUC1FacturadosBloqueEntero)/CONVERT(FLOAT,@VolTotalC1)
--			WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	CONVERT(FLOAT,MMBTUC2FacturadosBloqueEntero)/CONVERT(FLOAT,@VolTotalC2)		 
--			WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	CONVERT(FLOAT,MMBTUC3FacturadosBloqueEntero)/CONVERT(FLOAT,@VolTotalC3)
--			WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	CONVERT(FLOAT,MMBTUC4FacturadosBloqueEntero)/CONVERT(FLOAT,@VolTotalC4)
--		END							AS FACTORVOLUMEN1,
--		P.FactorVolumenC1,--FactorVolumenC2,FactorVolumenC3,FactorVolumenC4
--		FactorVolumenPrecioC1,
--		DescuentoPrecioBalanceoC1,
--		CASE
...
--		END		AS [NuevoPrecio],
--		CASE
--			WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	(MMBTUC1FacturadosBloqueEntero * ((pcC1/Total*Ingreso) + DescuentoPrecioBalanceoC1) )/@VolTotalC1
--			WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	(MMBTUC2FacturadosBloqueEntero * ((pcC2/Total*Ingreso) + DescuentoPrecioBalanceoC2) )/@VolTotalC2			 
--			WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	(MMBTUC3FacturadosBloqueEntero * ((pcC3/Total*Ingreso) + DescuentoPrecioBalanceoC3) )/@VolTotalC3
--			WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	(MMBTUC4FacturadosBloqueEntero * ((pcC4/Total*Ingreso) + DescuentoPrecioBalanceoC4) )/@VolTotalC4
--		END							AS NUEVAPONDERACION,
--		SE.SUMEnerC1
--	FROM
--		#PreciosGas2	P
--	CROSS JOIN
...
--	--CROSS JOIN #VolTotal	VT
--	CROSS JOIN #SumEner	SE
--	WHERE Tipo.IdTipoHidrocarburo = 10002
--END
...
BEGIN
	...
	BEGIN
		...
		WHERE
			...
			AND OC.IdTipoHidrocarburo BETWEEN	10002 AND 10005
		...
		SELECT
			...
			Tipo.CostoUnitarioComercializacion,
			--CASE
			--	WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	(pcC1 / Total * Ingreso) - Tipo.CostoUnitarioComercializacion
			--	WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	(pcC2 / Total * Ingreso) - Tipo.CostoUnitarioComercializacion
			--	WHEN	Tipo.IdTipoHidrocarburo  = 10004	THEN	(pcC3 / Total * Ingreso) - Tipo.CostoUnitarioComercializacion
			--	WHEN	Tipo.IdTipoHidrocarburo  = 10005	THEN	(pcC4 / Total * Ingreso) - Tipo.CostoUnitarioComercializacion
			--END							AS	PrecioPuntoMedicion,
			CASE
				WHEN	Tipo.IdTipoHidrocarburo  = 10002	THEN	((pcC1/Total*Ingreso) + DescuentoPrecioBalanceoC1) - Tipo.CostoUnitarioComercializacion
				WHEN	Tipo.IdTipoHidrocarburo  = 10003	THEN	((pcC2/Total*Ingreso) + DescuentoPrecioBalanceoC2) - Tipo.CostoUnitarioComercializacion
