...
AS
    BEGIN
        ...
        IF OBJECT_ID('tempdb..#ResultadoMontosCapexOpex', 'U') IS NOT NULL
            DROP TABLE #ResultadoMontosCapexOpex;

        IF OBJECT_ID('tempdb..#ResultadoMontosActividadSubActividad', 'U') IS NOT NULL
            DROP TABLE #ResultadoMontosActividadSubActividad;
        ...
        CREATE TABLE #ResultadoMontosCapexOpex
            (
                MontoTotalUSD FLOAT,
                CAPEX         BIT,
                OPEX          BIT,
        ...
        CREATE TABLE #ResultadoMontosActividadSubActividad
            (
                MontoUSDCapex FLOAT,
                MontoUSDOpex  FLOAT,
                Actividad     Varchar(500),
        ...
        SET @MesInicio = DATEFROMPARTS(YEAR(@MesInicio), MONTH(@MesInicio), 1);
        SET @MesFin = EOMONTH(DATEFROMPARTS(YEAR(@MesFin), MONTH(@MesFin), 1));
                ...
                MontoNotaCreditoUSD
                    ...
                    SELECT
                        ...
                        CASE
                            ...
                            ELSE
                                CONCAT(
                                          LTRIM(RTRIM(CO_ActividadPetroleraCNH.id_Actividad)), '_',
                        ...
                        CASE
                            ...
                            ELSE
                                CONCAT(
                                          LTRIM(RTRIM(CO_SubactividadPetrolera.[id_Sub-actividad])), '_',
                                          ISNULL(CO_SubactividadPetrolera.SubactividadPetrolera, '')
                                      )
                        END,              --Concepto de operación SUBACTIVIDAD
                        SUM(ABS(   CASE
                                       WHEN ISNULL(TTF.TCD, 0) = 0
                                           THEN 0
                                       WHEN ISNULL(TTF.MontoRegistro, 0) <> 0
                    ...
                    FROM
                        ...
                        LEFT JOIN
                            dbo.CO_SubactividadPetrolera (NOLOCK)
                                ON LPM.IdSubactividadPetrolera = CO_SubactividadPetrolera.IdSubactividadPetrolera
                        LEFT JOIN
                            CO_ActividadCIEP (NOLOCK)
                                ON LPM.IdActividad = CO_ActividadCIEP.IdActividad
                                   AND LPM.IdActividad = CO_ActividadCIEP.IdActividad
                    WHERE
                        DATEFROMPARTS(YEAR(CO_Registro.MesPresentacion), MONTH(CO_Registro.MesPresentacion), 1)
                        BETWEEN @MesInicio AND @MesFin
                        AND CO_Registro.IdEstado = @Aprobado
                        AND CO_Registro.CvTipoDocFacturacion = @TipoFactura
        ...
        INSERT INTO #ResultadoMontosActividadSubActividad
            (
                MontoUSDCapex,
                MontoUSDOpex,
                Actividad,
                        ...
                        LEFT JOIN
                            CO_ActividadCIEP (NOLOCK)
                                ON LPM.IdActividad = CO_ActividadCIEP.IdActividad
                                   AND LPM.IdActividad = CO_ActividadCIEP.IdActividad
                    WHERE
                        LPM.IdPresupuesto = @IdPresupuesto;


        UPDATE
            #ResultadoMontos
        SET
            Actividad = LTRIM(RTRIM(Actividad)),
            SubActividad = LTRIM(RTRIM(SubActividad));

        UPDATE
            #ResultadoMontosActividadSubActividad
        SET
            Actividad = LTRIM(RTRIM(Actividad)),
            SubActividad = LTRIM(RTRIM(SubActividad));

        INSERT INTO #ResultadoMontosCapexOpex
            (
                MontoTotalUSD,
                CAPEX,
                Actividad,
                SubActividad
                    ...
                    SELECT
                        ISNULL(SUM(ISNULL(MontoUSD, 0)), 0) - ISNULL(SUM(ISNULL(MontoNotaCreditoUSD, 0)), 0),
                        1,
                        Actividad,
                        SubActividad
                    FROM
        ...
        DELETE #ResultadoMontosActividadSubActividad
        WHERE
            Actividad IN (
                             '', '_', '-_-'
                         );
        ...
        UPDATE
            #ResultadoMontosActividadSubActividad
        SET
            #ResultadoMontosActividadSubActividad.MontoUSDCapex = #ResultadoMontosCapexOpex.MontoTotalUSD
        FROM
            #ResultadoMontosActividadSubActividad
            JOIN
                #ResultadoMontosCapexOpex
                    ON #ResultadoMontosActividadSubActividad.Actividad = #ResultadoMontosCapexOpex.Actividad
                       AND #ResultadoMontosActividadSubActividad.SubActividad = #ResultadoMontosCapexOpex.SubActividad
        WHERE
            CAPEX = 1
        ...
        UPDATE
            #ResultadoMontosActividadSubActividad
        SET
            #ResultadoMontosActividadSubActividad.MontoUSDOpex = #ResultadoMontosCapexOpex.MontoTotalUSD
        FROM
            #ResultadoMontosActividadSubActividad
            JOIN
                #ResultadoMontosCapexOpex
